...
$steampressvarstoreset = array('action','standalone','redirect','profile');
...
case 'adduser':
	...
	$loginthere = $steampressdb->get_var("SELECT user_login FROM $steampressdb->users WHERE user_login = '$user_login'");
    if ($loginthere)
	{
	...
	$user_ID = $steampressdb->get_var("SELECT ID FROM $steampressdb->users ORDER BY ID DESC LIMIT 1") + 1;
	...
	@steampress_mail(get_settings('admin_email'), '[' . get_settings('blogname') . '] New User Registration', $message);
...
case 'promote':
	...
	if ('up' == $prom)
	{
		...
		$sql="UPDATE $steampressdb->users SET user_level=$new_level WHERE ID = $id AND $new_level < $user_level";
	}
	elseif ('down' == $prom)
	{
		$new_level = $usertopromote_level - 1;
...
case 'delete':
	...
	$post_ids = $steampressdb->get_col("SELECT ID FROM $steampressdb->posts WHERE post_author = $id");
	if ($post_ids)
	{
		$post_ids = implode(',', $post_ids);
		
		// Delete comments, *backs
		$steampressdb->query("DELETE FROM $steampressdb->comments WHERE comment_post_ID IN ($post_ids)");
		// Clean cats
		$steampressdb->query("DELETE FROM $steampressdb->post2cat WHERE post_id IN ($post_ids)");
		// Clean post_meta
		$steampressdb->query("DELETE FROM $steampressdb->postmeta WHERE post_id IN ($post_ids)");
		// Clean links
		$steampressdb->query("DELETE FROM $steampressdb->links WHERE link_owner = $id");
		// Delete posts
		$steampressdb->query("DELETE FROM $steampressdb->posts WHERE post_author = $id");
	}

	// FINALLY, delete user
	$steampressdb->query("DELETE FROM $steampressdb->users WHERE ID = $id");
...
<div class="wrap">
  ...
  <table cellpadding="3" cellspacing="3" width="100%">
	...
	<?php
	$users = $steampressdb->get_results("SELECT ID FROM $steampressdb->users WHERE user_level > 0 ORDER BY ID");
	$style = '';
	foreach ($users as $user)
	{
		...
		$numposts = $steampressdb->get_var("SELECT COUNT(*) FROM $steampressdb->posts WHERE post_author = $user->ID and post_status = 'publish'");
		if (0 < $numposts)
		{
			$numposts = "<a href='edit.php?author=$user_data->ID' title='" . __('View posts') . "'>$numposts</a>";
		}
		echo "
<tr $style>
...
<?php
	$users = $steampressdb->get_results("SELECT * FROM $steampressdb->users WHERE user_level = 0 ORDER BY ID");
	if ($users)
	{
?>
<div class="wrap">
...
<?php printf(__('<p>Users can <a href="%s/steampress-register.php">register themselves</a> or you can manually create users here.</p>'), get_settings('siteurl')); ?>
