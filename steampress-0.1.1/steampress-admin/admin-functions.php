...
function return_categories_list( $parent = 0, $sortbyname = FALSE )
{
        ...
        $cats_tmp = $steampressdb->get_results("SELECT cat_id FROM $steampressdb->categories WHERE category_parent = $parent ORDER BY $sort");
...
function get_nested_categories($default = 0, $parent = 0) {
 global $post_ID, $mode, $steampressdb;

 if ($post_ID) {
   $checked_categories = $steampressdb->get_col("
     SELECT category_id
     FROM $steampressdb->categories, $steampressdb->post2cat
     WHERE $steampressdb->post2cat.category_id = cat_ID AND $steampressdb->post2cat.post_id = '$post_ID'
     ");
...
function cat_rows($parent = 0, $level = 0, $categories = 0) {
	global $steampressdb, $bgcolor, $user_level;
	if (!$categories)
	...
	if ($categories) {
		foreach ($categories as $category) {
			if ($category->category_parent == $parent) {
				...
				$count = $steampressdb->get_var("SELECT COUNT(post_id) FROM $steampressdb->post2cat WHERE category_id = $category->cat_ID");
...
function steampress_dropdown_cats($currentcat, $currentparent = 0, $parent = 0, $level = 0, $categories = 0) {
	global $steampressdb, $bgcolor;
	if (!$categories) {
		$categories = $steampressdb->get_results("SELECT * FROM $steampressdb->categories ORDER BY cat_name");
	}
	if ($categories) {
		foreach ($categories as $category) { if ($currentcat != $category->cat_ID && $parent == $category->category_parent) {
			...
			steampress_dropdown_cats($currentcat, $currentparent, $category->cat_ID, $level + 1, $categories);
		} }
	} else {
		return false;
	}
...
function steampress_create_thumbnail($file, $max_side, $effect = '') {
...
function has_meta($postid) {
	...
	return $steampressdb->get_results("
		SELECT meta_key, meta_value, meta_id, post_id
		FROM $steampressdb->postmeta
...
function add_meta($post_ID) {
	...
	$metakeyselect = $steampressdb->escape( stripslashes( trim($_POST['metakeyselect']) ) );
	$metakeyinput  = $steampressdb->escape( stripslashes( trim($_POST['metakeyinput']) ) );
	$metavalue     = $steampressdb->escape( stripslashes( trim($_POST['metavalue']) ) );
	...
	if (!empty($metavalue) && ((('#NONE#' != $metakeyselect) && !empty($metakeyselect)) || !empty($metakeyinput))) {
		...
		$result = $steampressdb->query("
				INSERT INTO $steampressdb->postmeta 
...
function delete_meta($mid) {
	...
	$result = $steampressdb->query("DELETE FROM $steampressdb->postmeta WHERE meta_id = '$mid'");
}

function update_meta($mid, $mkey, $mvalue) {
	global $steampressdb;

	return $steampressdb->query("UPDATE $steampressdb->postmeta SET meta_key = '$mkey', meta_value = '$mvalue' WHERE meta_id = '$mid'");
}
...
function check_admin_referer() {
  $adminurl = strtolower(get_settings('siteurl')).'/steampress-admin';
...
function save_mod_rewrite_rules() {
	global $is_apache, $steampress_rewrite;
	...
	if ($steampress_rewrite->using_index_permalinks())
		$usingpi = true;
	else
		$usingpi = false;

	if ( $writable && !$usingpi && $is_apache ) {
		$rules = explode("\n", $steampress_rewrite->mod_rewrite_rules());
...
function generate_page_rewrite_rules() {
	global $steampressdb;
	$posts = $steampressdb->get_results("SELECT ID, post_name FROM $steampressdb->posts WHERE post_status = 'static'");
...
if ( !strstr($_SERVER['HTTP_USER_AGENT'], 'Safari') ) :
	echo '
	<div id="quicktags">
	<a href="http://steampress.org/docs/reference/post/#quicktags" title="' .  __('Help with quicktags') . '">' . __('Quicktags') . '</a>:
...
function get_broken_themes() {
	...
	get_themes();
	return $steampress_broken_themes;
}
...
function parent_dropdown($default = 0, $parent = 0, $level = 0) {
	global $steampressdb, $post_ID;
	$items = $steampressdb->get_results("SELECT ID, post_parent, post_title FROM $steampressdb->posts WHERE post_parent = $parent AND post_status = 'static' ORDER BY menu_order");
...
function plugin_basename($file) {
	return preg_replace('#^.*steampress-content/plugins/#', '', $file);
}
...
$steampress_file_descriptions = array('index.php' => __('Main Template'),
															'steampress-layout.css' => __('Stylesheet'),
															'style.css' => __('Stylesheet'),
															'steampress-comments.php' => __('Comments Template'),
															'comments.php' => __('Comments Template'),
															'steampress-comments-popup.php' => __('Popup Comments Template'),
															'comments-popup.php' => __('Popup Comments Template'),
															'steampress-footer.php' => __('Footer Template'),
															'footer.php' => __('Footer Template'),
															'steampress-header.php' => __('Header Template'),
															'header.php' => __('Header Template'),
															'steampress-sidebar.php' => __('Sidebar Template'),
...
function get_file_description($file) {
	global $steampress_file_descriptions;

	if (isset($steampress_file_descriptions[$file])) {
		return $steampress_file_descriptions[$file];
	}
...
function get_plugins() {
	global $steampress_plugins;

	if (isset($steampress_plugins)) {
		return $steampress_plugins;
	}

	$steampress_plugins = array();
	$plugin_loc = 'steampress-content/plugins';
	$plugin_root = ABSPATH . $plugin_loc;

	// Files in steampress-content/plugins directory
	...
	foreach($plugin_files as $plugin_file) {
		...
		$steampress_plugins[plugin_basename($plugin_file)] = $plugin_data;
	}
