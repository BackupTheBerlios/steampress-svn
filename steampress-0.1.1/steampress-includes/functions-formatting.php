...
{
	global $steampress_htmltranswinuni;
	...
	// Fix Word pasting
	$content = strtr($content, $steampress_htmltranswinuni);
...
{
	global $steampress_smiliessearch, $steampress_smiliesreplace;
	$output = '';
	if (get_settings('use_smilies'))
	{
		...
		for ($i = 0; $i < $stop; $i++)
		{
			...
			{ // If it's not a tag
				$content = str_replace($steampress_smiliessearch, $steampress_smiliesreplace, $content);
			}
			$output .= $content;
		}
	}
...
// used by steampress-mail to handle charsets in email subjects
function steampress_iso_descrambler($string)
{
