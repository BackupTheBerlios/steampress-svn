...
function get_permalink($id = false)
{
	...
	if ($id)
	{
		$idpost = $steampressdb->get_row("SELECT ID, post_date, post_name, post_status, post_author FROM $steampressdb->posts WHERE ID = $id");
	}
	else
	{
		$idpost = $post;
...
{
	global $post, $steampress_rewrite;
...
{
	global $querystring_start, $querystring_equal, $steampress_rewrite;
	if (!$year) $year = gmdate('Y', time()+(get_settings('gmt_offset') * 3600));
	{
		$yearlink = $steampress_rewrite->get_year_permastruct();
	}
	if (!empty($yearlink))
	{
...
function get_month_link($year, $month)
{
	...
	$monthlink = $steampress_rewrite->get_month_permastruct();
	if (!empty($monthlink))
	{
...
function get_day_link($year, $month, $day)
{
	...
	$daylink = $steampress_rewrite->get_day_permastruct();
	if (!empty($daylink))
	{
...
function get_feed_link($feed='rss2')
{
	...
	{
		case 'rdf':
			$output = $feed_url .'/steampress-rdf.php';
			if ($do_perma)
			{
				$output = $feed_url . '/rdf/';
			}
			break;
		case 'rss':
			$output = $feed_url . '/steampress-rss.php';
			if ($do_perma)
			{
				$output = $feed_url . '/rss/';
			}
			break;
		case 'atom':
			$output = $feed_url .'/steampress-atom.php';
			if ($do_perma)
			{
			...
			break;		
		case 'comments_rss2':
			$output = $feed_url .'/steampress-commentsrss2.php';
			if ($do_perma)
			{
		...
		case 'rss2':
		default:
			$output = $feed_url .'/steampress-rss2.php';
			if ($do_perma)
			{
...
function edit_post_link($link = 'Edit This', $before = '', $after = '')
{
	...
	$location = get_settings('siteurl') . "/steampress-admin/post.php?action=edit&amp;post=$post->ID";
...
function edit_comment_link($link = 'Edit This', $before = '', $after = '')
{
	...
	$location = get_settings('siteurl') . "/steampress-admin/post.php?action=editcomment&amp;comment=$comment->comment_ID";
	echo "$before <a href='$location'>$link</a> $after";
}
...
function get_previous_post($in_same_cat = false, $excluded_categories = '')
{
	...
	return @$steampressdb->get_row("SELECT ID, post_title FROM $steampressdb->posts WHERE post_date < '$current_post_date' AND post_status = 'publish' $sqlcat $sql_exclude_cats ORDER BY post_date DESC LIMIT 1");
}
...
function get_next_post($in_same_cat = false, $excluded_categories = '')
{
	...
	return @$steampressdb->get_row("SELECT ID,post_title FROM $steampressdb->posts WHERE post_date > '$current_post_date' AND post_date < '$now' AND post_status = 'publish' $sqlcat $sql_exclude_cats AND ID != $post->ID ORDER BY post_date ASC LIMIT 1");
}
...
function previous_post($format='%', $previous='previous post: ', $title='yes', $in_same_cat='no', $limitprev=1, $excluded_categories='')
{
	...
	if(($posts_per_page == 1) || is_single())
	{
		...
		$limitprev--;
		$lastpost = @$steampressdb->get_row("SELECT ID, post_title FROM $steampressdb->posts WHERE post_date < '$current_post_date' AND post_status = 'publish' $sqlcat $sql_exclude_cats ORDER BY post_date DESC LIMIT $limitprev, 1");
		if ($lastpost)
		{
...
{
	global $posts_per_page, $post, $steampressdb;
	if(1 == $posts_per_page || is_single())
	{
		...
		$nextpost = @$steampressdb->get_row("SELECT ID,post_title FROM $steampressdb->posts WHERE post_date > '$current_post_date' AND post_date < '$now' AND post_status = 'publish' $sqlcat $sql_exclude_cats AND ID != $post->ID ORDER BY post_date ASC LIMIT $limitnext,1");
		if ($nextpost)
		{
...
{
	global $paged, $result, $request, $posts_per_page, $steampressdb;
	if (!$max_page)
	{
		preg_match('#FROM (.*) GROUP BY#', $request, $matches);
		$fromwhere = $matches[1];
		$numposts = $steampressdb->get_var("SELECT COUNT(ID) FROM $fromwhere");
...
{
	global $request, $posts_per_page, $steampressdb;
	if (! is_single())
	{
