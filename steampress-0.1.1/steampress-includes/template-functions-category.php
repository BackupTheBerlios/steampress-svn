...
{
	global $post, $steampressdb, $category_cache;
	...
	{
		$categories = $steampressdb->get_results("
			...
			WHERE $steampressdb->post2cat.category_id = cat_ID AND $steampressdb->post2cat.post_id = '$id'
			");
...
{
	global $steampressdb, $steampress_rewrite, $post, $querystring_start, $querystring_equal, $cache_categories;
	$cat_ID = $category_id;
	$catlink = $steampress_rewrite->get_category_permastruct();
...
function get_the_category_by_ID($cat_ID)
{
	...
	{
		$cat_name = $steampressdb->get_var("SELECT cat_name FROM $steampressdb->categories WHERE cat_ID = '$cat_ID'");
...
{
	global $cat, $steampressdb, $cache_categories;
	if (!$category)
	{
		$category = $cat;
	}
		...
		$selected=0, $hide=0)
{
	...
	$query = "
		...
		COUNT($steampressdb->post2cat.post_id) AS cat_count,
		DAYOFMONTH(MAX(post_date)) AS lastday, MONTH(MAX(post_date)) AS lastmonth
		FROM $steampressdb->categories LEFT JOIN $steampressdb->post2cat ON (cat_ID = category_id)
		LEFT JOIN $steampressdb->posts ON (ID = post_id)
		WHERE cat_ID > 0
		";
	if ($hide)
	{
	...
	$categories = $steampressdb->get_results($query);
...
// out of the steampress loop
function steampress_list_cats($args = '')
{
	parse_str($args, $r);
...
{
	global $steampressdb, $category_posts;
	global $querystring_start, $querystring_equal, $querystring_separator;
	...
	{
		$cat_counts = $steampressdb->get_results("	SELECT cat_ID,
		COUNT($steampressdb->post2cat.post_id) AS cat_count
		FROM $steampressdb->categories 
		INNER JOIN $steampressdb->post2cat ON (cat_ID = category_id)
		INNER JOIN $steampressdb->posts ON (ID = post_id)
	...
	if (intval($optiondates) == 1)
	{
		$cat_dates = $steampressdb->get_results("	SELECT cat_ID,
		DAYOFMONTH(MAX(post_date)) AS lastday, MONTH(MAX(post_date)) AS lastmonth
		FROM $steampressdb->categories 
		LEFT JOIN $steampressdb->post2cat ON (cat_ID = category_id)
		LEFT JOIN $steampressdb->posts ON (ID = post_id)
		WHERE post_status = 'publish' $exclusions
		GROUP BY category_id");
