...
class steampress_Query
{
	...
	function get_posts()
	{
		global $steampressdb, $pagenow, $request, $user_ID;
		...
		$steampress_posts_post_date_field = "post_date"; // "DATE_ADD(post_date, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)";
		...
		else
		{
			...
			$join = " LEFT JOIN $steampressdb->post2cat ON ($steampressdb->posts.ID = $steampressdb->post2cat.post_id) ";
		...
		if ('' != $q['category_name'])
		{
			...
			$tables = ", $steampressdb->post2cat, $steampressdb->categories";
			$join = " LEFT JOIN $steampressdb->post2cat ON ($steampressdb->posts.ID = $steampressdb->post2cat.post_id) LEFT JOIN $steampressdb->categories ON ($steampressdb->post2cat.category_id = $steampressdb->categories.cat_ID) ";
			$whichcat = " AND (category_nicename = '" . $q['category_name'] . "'";
			$q['cat'] = $steampressdb->get_var("SELECT cat_ID FROM $steampressdb->categories WHERE category_nicename = '" . $q['category_name'] . "'");
		...
		if ('' != $q['author_name'])
		{
			...
			$q['author'] = $steampressdb->get_var("SELECT ID FROM $steampressdb->users WHERE user_nicename='".$q['author_name']."'");
		...
		elseif (empty($q['nopaging']) && ! is_single())
		{
			$page = $q['paged'];
			if (empty($page))
			{
			...
			elseif ($q['what_to_show'] == 'days')
			{
				...
				$start_date = $steampressdb->get_var("SELECT max(post_date) FROM $steampressdb->posts $join WHERE (1=1) $where GROUP BY year(post_date), month(post_date), dayofmonth(post_date) ORDER BY post_date DESC LIMIT $startrow,1");
				$endrow = $startrow + $q['posts_per_page'] - 1;
				$end_date = $steampressdb->get_var("SELECT min(post_date) FROM $steampressdb->posts $join WHERE (1=1) $where GROUP BY year(post_date), month(post_date), dayofmonth(post_date) ORDER BY post_date DESC LIMIT $endrow,1");
		...
		$where .= " GROUP BY $steampressdb->posts.ID";
		$join = apply_filters('posts_join', $join);
		$request = " SELECT $distinct * FROM $steampressdb->posts $join WHERE 1=1".$where." ORDER BY post_" . $q['orderby'] . " $limits";
		...
		$this->posts = $steampressdb->get_results($request);
	...
	function steampress_Query ($query = '')
	{
		if (! empty($query))
		{
			$this->query($query);
...
if (! isset($steampress_query))
{
	$steampress_query = new steampress_Query();
}
...
class retrospam_mgr
{
	...
	function retrospam_mgr()
	{
		...
		$this->comment_list = $steampressdb->get_results("SELECT comment_ID AS ID, comment_content AS text, comment_approved AS approved, comment_author_url AS url, comment_author_ip AS ip, comment_author_email AS email FROM $steampressdb->comments ORDER BY comment_ID ASC");
	...
	function move_spam( $id_list )
	{
		...
		{
			if ( $steampressdb->query("update $steampressdb->comments set comment_approved = '0' where comment_ID = '$comment'") )
			{
				$cnt++;
			}
		}
...
class steampress_Rewrite
{
	...
	function generate_rewrite_rules($permalink_structure = '', $page = true, $feed = true, $forcomments = false)
	{
		...
		for ($j = 0; $j < $num_dirs; ++$j)
		{
			...
			{
				$post = 0;
				if (strstr($struct, '%postname%') || strstr($struct, '%post_id%') || (strstr($struct, '%year%') &&  strstr($struct, '%monthnum%') && strstr($struct, '%day%') && strstr($struct, '%hour%') && strstr($struct, '%minute') && strstr($struct, '%second%')))
				{
					$post = 1;
	...
	function steampress_rewrite_rules()
	{
...
// Make a global instance.
if (! isset($steampress_rewrite))
{
	$steampress_rewrite = new steampress_Rewrite();
}
