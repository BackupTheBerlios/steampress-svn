...
{
	global $all_links, $steampressdb;
	$links = $steampressdb->get_results("SELECT link_id, link_url FROM $steampressdb->links WHERE link_visible = 'Y' AND link_url <> ''");
...
function update_links()
{
	...
	while (list($id, $val) = each($all_links))
	{
		if ($val[1])
		{
			$steampressdb->query("UPDATE $steampressdb->links SET link_updated = '$val[1]' WHERE link_id = $val[0]");
		}
	} // end while
}
...
function get_weblogs_updatedfile()
{
	...
	$update = false;
	$file = ABSPATH . 'steampress-content/links-update-cache.xml';
	...
	if ($update)
	{
		...
		if ($a != false && count($a) && $a[0])
		{
			...
			$cachefp = fopen(ABSPATH . 'steampress-content/links-update-cache.xml', "w");
...
if (get_weblogs_updatedfile())
{
	...
	$fp = fopen(ABSPATH . 'steampress-content/links-update-cache.xml', "r")
