...
<p><a href="<?php echo get_settings('siteurl'); ?>/steampress-commentsrss2.php?p=<?php echo $post->ID; ?>"><?php _e("<abbr title=\"Really Simple Syndication\">RSS</abbr> feed for comments on this post."); ?></a></p>
...
$comments = $steampressdb->get_results("SELECT * FROM $steampressdb->comments WHERE comment_post_ID = $id AND comment_approved = '1' ORDER BY comment_date");
$commentstatus = $steampressdb->get_row("SELECT comment_status, post_password FROM $steampressdb->posts WHERE ID = $id");
if (!empty($commentstatus->post_password) && $_COOKIE['steampress-postpass_'. COOKIEHASH] != $commentstatus->post_password) {  // and it doesn't match the cookie
...
<form action="<?php echo get_settings('siteurl'); ?>/steampress-comments-post.php" method="post" id="commentform">
	<p>
...
<?php //} ?> 
<p class="credit"><?php timer_stop(1); ?> <?php echo sprintf(__("<cite>Powered by <a href=\"http://wordpress.org\" title=\"%s\"><strong>Wordpress</strong></a></cite>"),__("Powered by SteamPress, state-of-the-art semantic personal publishing platform.")); ?></p>
