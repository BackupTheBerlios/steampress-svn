...
function comments_template() {
	global $withcomments, $post, $spdb, $id, $comment;
	...
	if ( is_single() || is_page() || $withcomments ) :
		...
		$comments = $spdb->get_results("SELECT * FROM $spdb->comments WHERE comment_post_ID = '$post->ID' AND comment_approved = '1' ORDER BY comment_date");
		...
		if (file_exists($template)) {
			...
			include(ABSPATH . 'sp-comments.php');
		}
...
function get_comments_number( $comment_id ) {
	global $spdb, $comment_count_cache;
	$comment_id = (int) $comment_id;
	if (!isset($comment_count_cache[$comment_id])) 
		$number =  $spdb->get_var("SELECT COUNT(*) FROM $spdb->comments WHERE comment_post_ID = '$comment_id' AND comment_approved = '1'");
	else 
...
function comment_link_atom() {
	echo get_comments_link();
}

function comments_popup_script($width=400, $height=400, $file='') {
    global $spcommentspopupfile, $sptrackbackpopupfile, $sppingbackpopupfile, $spcommentsjavascript;

		if (empty ($file)) {
			$template = TEMPLATEPATH . '/comments-popup.php';
			if (file_exists($template)) {
				$spcommentspopupfile = str_replace(ABSPATH, '', $template);
			} else {
				$spcommentspopupfile = 'sp-comments-popup.php';
			}
		} else {
			$spcommentspopupfile = $file;
		}

    $spcommentsjavascript = 1;
    $javascript = "<script type='text/javascript'>\nfunction wpopen (macagna) {\n    window.open(macagna, '_blank', 'width=$width,height=$height,scrollbars=yes,status=yes');\n}\n</script>\n";
    echo $javascript;
}

function comments_popup_link($zero='No Comments', $one='1 Comment', $more='% Comments', $CSSclass='', $none='Comments Off') {
    global $id, $spcommentspopupfile, $spcommentsjavascript, $post, $spdb;
    global $querystring_start, $querystring_equal, $querystring_separator;
    global $comment_count_cache;

	if (! is_single() && ! is_page()) {
    if ('' == $comment_count_cache["$id"]) {
        $number = $spdb->get_var("SELECT COUNT(comment_ID) FROM $spdb->comments WHERE comment_post_ID = $id AND comment_approved = '1';");
    } else {
    ...
    if (0 == $number && 'closed' == $post->comment_status && 'closed' == $post->ping_status) {
        ...
        if (!empty($post->post_password)) { // if there's a password
            if ($_COOKIE['sp-postpass_'.COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
                echo('Enter your password to view comments');
                return;
            }
        }
        echo '<a href="';
        if ($spcommentsjavascript) {
            echo get_settings('siteurl') . '/' . $spcommentspopupfile.$querystring_start.'p'.$querystring_equal.$id.$querystring_separator.'c'.$querystring_equal.'1';
            //echo get_permalink();
...
function comments_rss_link($link_text = 'Comments RSS', $commentsrssfilename = 'sp-commentsrss2.php') {
	$url = comments_rss($commentsrssfilename);
	echo "<a href='$url'>$link_text</a>";
}

function comments_atom_link($link_text = 'Comments Atom', $commentsrssfilename = 'sp-commentsatom.php') {
	$url = comments_rss($commentsrssfilename);
	echo "<a href='$url'>$link_text</a>";
}

function comments_rss($commentsrssfilename = 'sp-commentsrss2.php') {
	global $id;
	global $querystring_start, $querystring_equal, $querystring_separator;
...
function comments_atom($commentsrssfilename = 'sp-commentsatom.php') {
	global $id;
	global $querystring_start, $querystring_equal, $querystring_separator;
...
function get_trackback_url() {
	global $id;
	$tb_url = get_settings('siteurl') . '/sp-trackback.php/' . $id;
...
function trackback_rdf($timezone = 0) {
	...
	the_permalink();
	echo '"'."\n";
	echo '    dc:title="'.str_replace('--', '&#x2d;&#x2d;', sptexturize(strip_tags(get_the_title()))).'"'."\n";
...
function get_lastcommentmodified($timezone = 'server') {
	global $tablecomments, $cache_lastcommentmodified, $pagenow, $spdb;
	...
	if ( !isset($cache_lastcommentmodified[$timezone]) ) {
		switch(strtolower($timezone)) {
			case 'gmt':
				$lastcommentmodified = $spdb->get_var("SELECT comment_date_gmt FROM $tablecomments WHERE comment_date_gmt <= '$now' ORDER BY comment_date_gmt DESC LIMIT 1");
				break;
			case 'blog':
				$lastcommentmodified = $spdb->get_var("SELECT comment_date FROM $tablecomments WHERE comment_date_gmt <= '$now' ORDER BY comment_date_gmt DESC LIMIT 1");
				break;
			case 'server':
				$lastcommentmodified = $spdb->get_var("SELECT DATE_ADD(comment_date_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $tablecomments WHERE comment_date_gmt <= '$now' ORDER BY comment_date_gmt DESC LIMIT 1");
				break;
		}
...
function get_commentdata($comment_ID,$no_cache=0,$include_unapproved=false) { // less flexible, but saves DB queries
	global $postc,$id,$commentdata, $spdb;
	if ($no_cache) {
		$query = "SELECT * FROM $spdb->comments WHERE comment_ID = '$comment_ID'";
		if (false == $include_unapproved) {
		    $query .= " AND comment_approved = '1'";
		}
    		$myrow = $spdb->get_row($query, ARRAY_A);
	} else {
...
function discover_pingback_server_uri($url, $timeout_bytes = 2048) {
	...
	$request = "GET $path HTTP/1.1\r\nHost: $host\r\nUser-Agent: SteamPress/$sp_version PHP/" . phpversion() . "\r\n\r\n";
	ob_end_flush();
	fputs($fp, $request);
 ...
 */
function sp_set_comment_status($comment_id, $comment_status) {
    global $spdb;

    switch($comment_status) {
		case 'hold':
			$query = "UPDATE $spdb->comments SET comment_approved='0' WHERE comment_ID='$comment_id' LIMIT 1";
		break;
		case 'approve':
			$query = "UPDATE $spdb->comments SET comment_approved='1' WHERE comment_ID='$comment_id' LIMIT 1";
		break;
		case 'delete':
			$query = "DELETE FROM $spdb->comments WHERE comment_ID='$comment_id' LIMIT 1";
		break;
		default:
			return false;
    }
    ...
    if ($spdb->query($query)) {
		do_action('sp_set_comment_status', $comment_id);
		return true;
    } else {
		return false;
    }
 ...
 */
function sp_get_comment_status($comment_id) {
    global $spdb;
    
    $result = $spdb->get_var("SELECT comment_approved FROM $spdb->comments WHERE comment_ID='$comment_id' LIMIT 1");
    if ($result == NULL) {
        return "deleted";
    } else if ($result == "1") {
        return "approved";
...
function sp_notify_postauthor($comment_id, $comment_type='comment') {
    global $spdb;
    global $querystring_start, $querystring_equal, $querystring_separator;
    
    $comment = $spdb->get_row("SELECT * FROM $spdb->comments WHERE comment_ID='$comment_id' LIMIT 1");
    $post = $spdb->get_row("SELECT * FROM $spdb->posts WHERE ID='$comment->comment_post_ID' LIMIT 1");
    $user = $spdb->get_row("SELECT * FROM $spdb->users WHERE ID='$post->post_author' LIMIT 1");
	...
	@sp_mail($user->user_email, $subject, $notify_message, $message_headers);
...
/* sp_notify_moderator
   ...
   always returns true
 */
function sp_notify_moderator($comment_id) {
    global $spdb;
    ...
    $comment_author_domain = gethostbyaddr($comment->comment_author_IP);
    $comments_waiting = $spdb->get_var("SELECT count(comment_ID) FROM $spdb->comments WHERE comment_approved = '0'");
    ...
    $notify_message .= "Whois  : http://ws.arin.net/cgi-bin/whois.pl?queryinput=$comment->comment_author_IP\r\n";
    $notify_message .= "Comment:\r\n".$comment->comment_content."\r\n\r\n";
    $notify_message .= "To approve this comment, visit: " . get_settings('siteurl') . "/sp-admin/post.php?action=mailapprovecomment&p=".$comment->comment_post_ID."&comment=$comment_id\r\n";
    $notify_message .= "To delete this comment, visit: " . get_settings('siteurl') . "/sp-admin/post.php?action=confirmdeletecomment&p=".$comment->comment_post_ID."&comment=$comment_id\r\n";
    $notify_message .= "Currently $comments_waiting comments are waiting for approval. Please visit the moderation panel:\r\n";
    $notify_message .= get_settings('siteurl') . "/sp-admin/moderation.php\r\n";
    ...
    @sp_mail($admin_email, $subject, $notify_message, $message_headers);
...
function check_comment($author, $email, $url, $comment, $user_ip, $user_agent) {
	...
	if ( 1 == get_settings('comment_whitelist')) {
		if( $author != '' && $email != '' ) {
		    $ok_to_comment = $spdb->get_var("SELECT comment_approved FROM $spdb->comments WHERE comment_author_email = '$email' and comment_approved = '1' ");
