...
function get_the_password_form() {
    $output = '<form action="' . get_settings('siteurl') . '/sp-pass.php" method="post">
...
function get_the_content($more_link_text = '(more...)', $stripteaser = 0, $more_file = '') {
    ...
    if (!empty($post->post_password)) { // if there's a password
        if (stripslashes($_COOKIE['sp-postpass_'.COOKIEHASH]) != $post->post_password) {  // and it doesn't match the cookie
...
function sp_list_pages($args = '') {
	...
	$pages = $spdb->get_results("SELECT ID, post_title FROM $spdb->posts WHERE post_status = 'static' ORDER BY post_" . $r['sort_column'] . " " . $r['sort_order'] = 'asc');
	...
	foreach ($pages as $page) {
		...
		echo '<a href="' . get_page_link($page->ID) . '" title="' . sp_specialchars($title) . '">' . $title . '</a>';
		echo '</li>';
	}
}
