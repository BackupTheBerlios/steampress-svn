...
function get_linksbyname($cat_name = "noname", $before = '', $after = '<br />',
    ...
    global $spdb;
    $cat_id = -1;
    $results = $spdb->get_results("SELECT cat_id FROM $spdb->linkcategories WHERE cat_name='$cat_name'");
    if ($results) {
...
/** function sp_get_linksbyname()
 ** Gets the links associated with the named category.
 ** Parameters:
 **   category (no default)  - The category to use.
 **/
function sp_get_linksbyname($category) {
    global $spdb;

    $cat = $spdb->get_row("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
         . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
         . " text_after_all, list_limit FROM $spdb->linkcategories WHERE cat_name='$category'");
    if ($cat) {
        if ($cat->sort_desc == 'Y') {
...
} // end sp_get_linksbyname

/** function sp_get_links()
 ** Gets the links associated with category n.
 ** Parameters:
 **   category (no default)  - The category to use.
 **/
function sp_get_links($category) {
    global $spdb;

    $cat = $spdb->get_row("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
         . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
         . " text_after_all, list_limit FROM $spdb->linkcategories WHERE cat_id=$category");
    if ($cat) {
        if ($cat->sort_desc == 'Y') {
            $cat->sort_order = '_'.$cat->sort_order;
        }
...
function get_links($category = -1, $before = '', $after = '<br />',
    ...
    foreach ($results as $row) {
        ...
        if (($row->link_url != null) && ($row->link_url != '')) {
            $the_link = sp_specialchars($row->link_url);
        }
        $rel = $row->link_rel;
        if ($rel != '') {
            $rel = " rel='$rel'";
        }
        $desc = sp_specialchars($row->link_description, ENT_QUOTES);
        $name = sp_specialchars($row->link_name, ENT_QUOTES);
...
function get_linkobjects($category = -1, $orderby = 'name', $limit = -1) {
    ...
    $sql = "SELECT * FROM $spdb->links WHERE link_visible = 'Y'";
    if ($category != -1) {
...
function get_linkcatname($id = 0) {
    ...
    $cat_name = '';
    if ('' != $id) {
        $cat_name = $spdb->get_var("SELECT cat_name FROM $spdb->linkcategories WHERE cat_id=$id");
    }
    return $cat_name;
}
...
function get_autotoggle($id = 0) {
    global $spdb;
    $auto_toggle = $spdb->get_var("SELECT auto_toggle FROM $spdb->linkcategories WHERE cat_id=$id");
    if ('' == $auto_toggle)
        $auto_toggle = 'N';
    return $auto_toggle;
}
...
function links_popup_script($text = 'Links', $width=400, $height=400,
   ...
   if ($count == true) {
      $counts = $spdb->get_var("SELECT count(*) FROM $spdb->links");
   }
 ...
 * settings in $spdb->linkcategories and output it as a nested
 * HTML unordered list.
 *
 * Parameters:
...
function get_links_list($order = 'name', $hide_if_empty = 'obsolete') {
	...
	$cats = $spdb->get_results("
		SELECT DISTINCT link_category, cat_name, show_images, 
			show_description, show_rating, show_updated, sort_order, 
			sort_desc, list_limit
		FROM `$spdb->links` 
		LEFT JOIN `$spdb->linkcategories` ON (link_category = cat_id)
		WHERE link_visible =  'Y'
			AND list_limit <> 0
	...
	if ($cats) {
		foreach ($cats as $cat) {
			...
			echo '	<dt id="'.sanitize_title($cat['cat_name']).'">' . $cat['cat_name'] . "</dt>\n";
