...
function sp_specialchars( $text, $quotes = 0 ) {
...
function convert_chars($content, $flag = 'obsolete') { 
	...
	// Fix Word pasting
	$content = strtr($content, $sp_htmltranswinuni);
...
function balanceTags($text, $is_comment = 0) {
	...
	# SP bug fix for comments - in case you REALLY meant to type '< !--'
	$text = str_replace('< !--', '<    !--', $text);
	# SP bug fix for LOVE <3 (and other situations with '<' before a number)
	...
	// SP fix for the bug with HTML comments
...
function convert_smilies($text) {
	global $sp_smiliessearch, $sp_smiliesreplace;
    $output = '';
	if (get_settings('use_smilies')) {
		...
		for ($i = 0; $i < $stop; $i++) {
			...
			if ((strlen($content) > 0) && ('<' != $content{0})) { // If it's not a tag
				$content = str_replace($sp_smiliessearch, $sp_smiliesreplace, $content);
			}
			$output .= $content;
		}
	} else {
...
// used by sp-mail to handle charsets in email subjects
function sp_iso_descrambler($string) {
