...
function comments_number($zero='No Comments', $one='1 Comment', $more='% Comments', $number='') {
    global $id, $comment, $spdb, $comment_count_cache;
    if ('' == $comment_count_cache["$id"]) $number = $spdb->get_var("SELECT COUNT(*) FROM $spdb->comments WHERE comment_post_ID = $id AND comment_approved = '1'");
...
function comments_popup_script($width=400, $height=400, $file='sp-comments-popup.php') {
...
function comments_atom_link($link_text='Comments Atom Feed', $commentsrssfilename = 'sp-commentsatom.php') {
	$url = comments_rss($commentsrssfilename);
	echo "<a href='$url'>$link_text</a>";
}
...
function comment_author_rss() {
	...
	if (empty($comment->comment_author)) {
		...
		echo sp_specialchars(apply_filters('comment_author', $comment->comment_author));
	}
}
...
function comment_text_rss() {
	...
	$comment_text = strip_tags($comment_text);
	$comment_text = sp_specialchars($comment_text);
	echo $comment_text;
}
