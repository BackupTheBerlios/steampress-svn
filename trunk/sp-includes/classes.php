...
class SP_Query {
	...
	function parse_query ($query) {
		...
		if (strstr($_SERVER['PHP_SELF'], 'sp-admin/')) {
			$this->is_admin = true;
		}
	...
	function get_posts() {
		global $spdb, $pagenow, $request, $user_ID;
		...
		$sp_posts_post_date_field = "post_date"; // "DATE_ADD(post_date, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)";
		...
		if ((empty($q['cat'])) || ($q['cat'] == 'all') || ($q['cat'] == '0') || 
			...
			$join = " LEFT JOIN $spdb->post2cat ON ($spdb->posts.ID = $spdb->post2cat.post_id) ";
		...
		if ('' != $q['category_name']) {
			...
			$tables = ", $spdb->post2cat, $spdb->categories";
			$join = " LEFT JOIN $spdb->post2cat ON ($spdb->posts.ID = $spdb->post2cat.post_id) LEFT JOIN $spdb->categories ON ($spdb->post2cat.category_id = $spdb->categories.cat_ID) ";
			$whichcat = " AND (category_nicename = '" . $q['category_name'] . "'";
			$q['cat'] = $spdb->get_var("SELECT cat_ID FROM $spdb->categories WHERE category_nicename = '" . $q['category_name'] . "'");
		...
		if ('' != $q['author_name']) {
			...
			$q['author'] = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_nicename='".$q['author_name']."'");
		...
		if ( !empty($q['postend']) && ($q['postend'] > $q['poststart']) ) {
			...
			if (($q['what_to_show'] == 'posts')) {
				...
				$start_date = $spdb->get_var("SELECT max(post_date) FROM $spdb->posts $join WHERE (1=1) $where GROUP BY year(post_date), month(post_date), dayofmonth(post_date) ORDER BY post_date DESC LIMIT $startrow,1");
				$endrow = $startrow + $q['posts_per_page'] - 1;
				$end_date = $spdb->get_var("SELECT min(post_date) FROM $spdb->posts $join WHERE (1=1) $where GROUP BY year(post_date), month(post_date), dayofmonth(post_date) ORDER BY post_date DESC LIMIT $endrow,1");
		...
		$where .= " GROUP BY $spdb->posts.ID";
		$join = apply_filters('posts_join_paged', $join);
		$request = " SELECT $distinct * FROM $spdb->posts $join WHERE 1=1".$where." ORDER BY post_" . $q['orderby'] . " $limits";
		...
		$this->posts = $spdb->get_results($request);
...
class retrospam_mgr {
	...
	function retrospam_mgr() {
		...
		$this->comment_list = $spdb->get_results("SELECT comment_ID AS ID, comment_content AS text, comment_approved AS approved, comment_author_url AS url, comment_author_ip AS ip, comment_author_email AS email FROM $spdb->comments ORDER BY comment_ID ASC");
	...
	function move_spam( $id_list ) {
		...
		foreach ( $id_list as $comment ) {
			if ( $spdb->query("update $spdb->comments set comment_approved = '0' where comment_ID = '$comment'") ) {
				$cnt++;
			}
		}
