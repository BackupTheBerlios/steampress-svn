...
function preload_links() {
	global $all_links, $spdb;
	$links = $spdb->get_results("SELECT link_id, link_url FROM $spdb->links WHERE link_visible = 'Y' AND link_url <> ''");
...
function update_links() {
	...
	while (list($id, $val) = each($all_links)) {
		if ($val[1]) {
			$spdb->query("UPDATE $spdb->links SET link_updated = '$val[1]' WHERE link_id = $val[0]");
		}
	} // end while
}
...
function get_weblogs_updatedfile() {
	...
	$update = false;
	$file = ABSPATH . 'sp-content/links-update-cache.xml';
	...
	if ($update) {
		...
		if ($a != false && count($a) && $a[0]) {
			...
			$cachefp = fopen(ABSPATH . 'sp-content/links-update-cache.xml', "w");
...
if (get_weblogs_updatedfile()) {
	...
	$fp = fopen(ABSPATH . 'sp-content/links-update-cache.xml', "r")
