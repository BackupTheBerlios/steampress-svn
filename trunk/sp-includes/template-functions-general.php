...
function get_bloginfo($show='') {
	...
	case 'stylesheet_url':
		...
		if (empty($output) || $output == 'default') {
			$output = get_settings('siteurl') . "/sp-layout.css";
		} else {
			$output = get_settings('siteurl') . "/sp-content/themes/$output/style.css";
		}
		break;
	case 'stylesheet_directory':
		...
		if (empty($output) || $output == 'default') {
			...
			$output = get_settings('siteurl') . "/sp-content/themes/$output";
		}
		break;
...
function sp_title($sep = '&raquo;', $display = true) {
    global $spdb;
    ...
    if (!empty($category_name)) {
        ...
        $title = $spdb->get_var("SELECT cat_name FROM $spdb->categories WHERE category_nicename = '$category_name'");
    }
...
function single_post_title($prefix = '', $display = true) {
    ...
    if (intval($p) || '' != $name) {
        if (!$p) {
            $p = $spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_name = '$name'");
        }
...
function get_archives_link($url, $text, $format = 'html', $before = '', $after = '') {
	$text = sptexturize($text);
    $title_text = sp_specialchars($text, 1);
	...
	if ('link' == $format) {
		...
		return "\t<dd>$before<a href='$url' title='$title_text'>$text</a>$after</dd>\n";
	} else { // custom
		return "\t$before<a href='$url' title='$title_text'>$text</a>$after\n";
	}
}

function sp_get_archives($args = '') {
	parse_str($args, $r);
...
function get_archives($type='', $limit='', $format='html', $before = '', $after = '', $show_post_count = false) {
    global $querystring_start, $querystring_equal, $querystring_separator, $month, $spdb;
    ...
    if ('monthly' == $type) {
        $arcresults = $spdb->get_results("SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $spdb->posts WHERE post_date < '$now' AND post_status = 'publish' GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date DESC" . $limit);
        if ($arcresults) {
            $afterafter = $after;
        ...
        $arcresults = $spdb->get_results("SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth` FROM $spdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
        if ($arcresults) {
            foreach ($arcresults as $arcresult) {
	...
	$start_of_week = get_settings('start_of_week');
        $arcresults = $spdb->get_results("SELECT DISTINCT WEEK(post_date, $start_of_week) AS `week`, YEAR(post_date) AS yr, DATE_FORMAT(post_date, '%Y-%m-%d') AS yyyymmdd FROM $spdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
        $arc_w_last = '';
        if ($arcresults) {
            foreach ($arcresults as $arcresult) {
        ...
        $arcresults = $spdb->get_results("SELECT ID, post_date, post_title FROM $spdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
        if ($arcresults) {
            foreach ($arcresults as $arcresult) {
...
function get_calendar($daylength = 1) {
    global $spdb, $m, $monthnum, $year, $timedifference, $month, $month_abbrev, $weekday, $weekday_initial, $weekday_abbrev, $posts;

    // Quick check. If we have no posts at all, abort!
    if (!$posts) {
        $gotsome = $spdb->get_var("SELECT ID from $spdb->posts WHERE post_status = 'publish' ORDER BY post_date DESC LIMIT 1");
        if (!$gotsome)
            return;
    }
    ...
    if (!empty($monthnum) && !empty($year)) {
        ...
        $thismonth = $spdb->get_var("SELECT DATE_FORMAT((DATE_ADD('${thisyear}0101', INTERVAL $d DAY) ), '%m')");
    } elseif (!empty($m)) {
    ...
    $previous = $spdb->get_row("SELECT DISTINCT MONTH(post_date) AS month, YEAR(post_date) AS year
            FROM $spdb->posts
            WHERE post_date < '$thisyear-$thismonth-01'
            AND post_status = 'publish'
                              ORDER BY post_date DESC
                              LIMIT 1");
    $next = $spdb->get_row("SELECT  DISTINCT MONTH(post_date) AS month, YEAR(post_date) AS year
            FROM $spdb->posts
            WHERE post_date >  '$thisyear-$thismonth-01'
    ...
    // Get days with posts
    $dayswithposts = $spdb->get_results("SELECT DISTINCT DAYOFMONTH(post_date)
            FROM $spdb->posts WHERE MONTH(post_date) = $thismonth
    ...
    $ak_post_titles = $spdb->get_results("SELECT post_title, DAYOFMONTH(post_date) as dom "
                                         ."FROM $spdb->posts "
    ...
    if ($ak_post_titles) {
        foreach ($ak_post_titles as $ak_post_title) {
            ...
            if (empty($ak_titles_for_day["$ak_post_title->dom"])) { // first one
                $ak_titles_for_day["$ak_post_title->dom"] = str_replace('"', '&quot;', sptexturize($ak_post_title->post_title));
            } else {
                $ak_titles_for_day["$ak_post_title->dom"] .= $ak_title_separator . str_replace('"', '&quot;', sptexturize($ak_post_title->post_title));
            }
        }
    }
