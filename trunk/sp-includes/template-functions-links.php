...
function get_permalink($id = false) {
	...
	if ($id) {
		$idpost = $spdb->get_row("SELECT ID, post_date, post_name, post_status, post_author FROM $spdb->posts WHERE ID = $id");
	} else {
		$idpost = $post;
	}
...
function get_year_link($year) {
	global $querystring_start, $querystring_equal, $sp_rewrite;
    if (!$year) $year = gmdate('Y', time()+(get_settings('gmt_offset') * 3600));
		$yearlink = $sp_rewrite->get_year_permastruct();
    if (!empty($yearlink)) {
...
function get_month_link($year, $month) {
    ...
    if (!$month) $month = gmdate('m', time()+(get_settings('gmt_offset') * 3600));
		$monthlink = $sp_rewrite->get_month_permastruct();
    if (!empty($monthlink)) {
...
function get_day_link($year, $month, $day) {
    ...
    if (!$day) $day = gmdate('j', time()+(get_settings('gmt_offset') * 3600));
		...
		$daylink = $sp_rewrite->get_day_permastruct();
    if (!empty($daylink)) {
...
function get_feed_link($feed='rss2') {
    ...
    switch($feed) {
        case 'rdf':
            $output = $feed_url .'/sp-rdf.php';
            if ($do_perma) {
                $output = $feed_url . '/rdf/';
            }
            break;
        case 'rss':
            $output = $feed_url . '/sp-rss.php';
            if ($do_perma) {
                $output = $feed_url . '/rss/';
            }
            break;
        case 'atom':
            $output = $feed_url .'/sp-atom.php';
            if ($do_perma) {
                $output = $feed_url . '/atom/';
            }
            break;        
        case 'comments_rss2':
            $output = $feed_url .'/sp-commentsrss2.php';
            if ($do_perma) {
...
function edit_post_link($link = 'Edit This', $before = '', $after = '') {
    ...
    $location = get_settings('siteurl') . "/sp-admin/post.php?action=edit&amp;post=$post->ID";
...
function edit_comment_link($link = 'Edit This', $before = '', $after = '') {
    ...
    $location = get_settings('siteurl') . "/sp-admin/post.php?action=editcomment&amp;comment=$comment->comment_ID";
    echo "$before <a href='$location'>$link</a> $after";
}
...
function get_previous_post($in_same_cat = false, $excluded_categories = '') {
    ...
    return @$spdb->get_row("SELECT ID, post_title FROM $spdb->posts WHERE post_date < '$current_post_date' AND post_status = 'publish' $sqlcat $sql_exclude_cats ORDER BY post_date DESC LIMIT 1");
}
...
function get_next_post($in_same_cat = false, $excluded_categories = '') {
    ...
    return @$spdb->get_row("SELECT ID,post_title FROM $spdb->posts WHERE post_date > '$current_post_date' AND post_date < '$now' AND post_status = 'publish' $sqlcat $sql_exclude_cats AND ID != $post->ID ORDER BY post_date ASC LIMIT 1");
}
...
function previous_post($format='%', $previous='previous post: ', $title='yes', $in_same_cat='no', $limitprev=1, $excluded_categories='') {
    ...
    if(($posts_per_page == 1) || is_single()) {
        ...
        $limitprev--;
        $lastpost = @$spdb->get_row("SELECT ID, post_title FROM $spdb->posts WHERE post_date < '$current_post_date' AND post_status = 'publish' $sqlcat $sql_exclude_cats ORDER BY post_date DESC LIMIT $limitprev, 1");
        if ($lastpost) {
            $string = '<a href="'.get_permalink($lastpost->ID).'">'.$previous;
            if ($title == 'yes') {
                $string .= sptexturize($lastpost->post_title);
            }
            $string .= '</a>';
...
function next_post($format='%', $next='next post: ', $title='yes', $in_same_cat='no', $limitnext=1, $excluded_categories='') {
    global $posts_per_page, $post, $spdb;
    if(1 == $posts_per_page || is_single()) {
        ...
        $nextpost = @$spdb->get_row("SELECT ID,post_title FROM $spdb->posts WHERE post_date > '$current_post_date' AND post_date < '$now' AND post_status = 'publish' $sqlcat $sql_exclude_cats AND ID != $post->ID ORDER BY post_date ASC LIMIT $limitnext,1");
        if ($nextpost) {
            $string = '<a href="'.get_permalink($nextpost->ID).'">'.$next;
            if ($title=='yes') {
                $string .= sptexturize($nextpost->post_title);
            }
            $string .= '</a>';
            $format = str_replace('%', $string, $format);
            echo $format;
...
function next_posts_link($label='Next Page &raquo;', $max_page=0) {
    global $paged, $result, $request, $posts_per_page, $spdb;
    if (!$max_page) {
        preg_match('#FROM (.*) GROUP BY#', $request, $matches);
        $fromwhere = $matches[1];
        $numposts = $spdb->get_var("SELECT COUNT(ID) FROM $fromwhere");
...
function posts_nav_link($sep=' &#8212; ', $prelabel='&laquo; Previous Page', $nxtlabel='Next Page &raquo;') {
	global $request, $posts_per_page, $spdb;
	if (! is_single()) {
