...
function the_author_posts_link($idmode='') {
    ...
    echo '<a href="' . get_author_link(0, $authordata->ID, $authordata->user_nicename) . '" title="' . sprintf(__("Posts by %s"), sp_specialchars(the_author($idmode, false))) . '">' . the_author($idmode, false) . '</a>';
}


function get_author_link($echo = false, $author_id, $author_nicename) {
	global $spdb, $sp_rewrite, $post, $querystring_start, $querystring_equal, $cache_userdata;
    $auth_ID = $author_id;
    $link = $sp_rewrite->get_author_permastruct();
...
function list_authors($optioncount = false, $exclude_admin = true, $show_fullname = false, $hide_empty = true, $feed = '', $feed_image = '') {
    global $spdb, $blogfilename;

    $query = "SELECT ID, user_nickname, user_firstname, user_lastname, user_nicename from $spdb->users " . ($exclude_admin ? "WHERE user_nickname <> 'admin' " : '') . "ORDER BY user_nickname";
    $authors = $spdb->get_results($query);
    ...
    foreach($authors as $author) {
        ...
        if ($posts == 0) {
            ...
            $link = '<a href="' . get_author_link(0, $author->ID, $author->user_nicename) . '" title="' . sprintf(__("Posts by %s"), sp_specialchars($author->user_nickname)) . '">' . $name . '</a>';
