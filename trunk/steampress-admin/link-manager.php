...
function xfn_check($class, $value = '', $type = 'check')
{
	...
	if ('' == $value)
	{
		if ('family' == $class && !strstr($link_rel, 'child') && !strstr($link_rel, 'parent') && !strstr($link_rel, 'sibling') && !strstr($link_rel, 'spouse') && !strstr($link_rel, 'kin'))
		{
			echo ' checked="checked"';
		}
		if ('friendship' == $class && !strstr($link_rel, 'friend') && !strstr($link_rel, 'acquaintance') && !strstr($link_rel, 'contact') )
		{
			echo ' checked="checked"';
		}
		if ('geographical' == $class && !strstr($link_rel, 'co-resident') && !strstr($link_rel, 'neighbor') )
		{
			echo ' checked="checked"';
		}
...
function category_dropdown($fieldname, $selected = 0)
{
	...
	$results = $steampressdb->get_results("SELECT cat_id, cat_name, auto_toggle FROM $steampressdb->linkcategories ORDER BY cat_id");
...
$steampressvarstoreset = array('action','cat_id', 'linkurl', 'name', 'image',
...
switch ($action)
{
	...
	$results = $steampressdb->get_results("SELECT link_id, link_owner, user_level FROM $steampressdb->links LEFT JOIN $steampressdb->users ON link_owner = ID WHERE link_id in ($all_links)");
	foreach ($results as $row)
	{
		if (($user_level >= $row->user_level))
		{ // ok to proceed
	...
	$q = $steampressdb->query("update $steampressdb->links SET link_owner='$newowner' WHERE link_id IN ($all_links)");
	...
	$all_links = join(',', $linkcheck);
	$results = $steampressdb->get_results("SELECT link_id, link_visible FROM $steampressdb->links WHERE link_id in ($all_links)");
	foreach ($results as $row)
	{
	...
	if (count($ids_to_turnoff))
	{
		...
		$q = $steampressdb->query("update $steampressdb->links SET link_visible='N' WHERE link_id IN ($all_linksoff)");
	}

	if (count($ids_to_turnon))
	{
		$all_linkson = join(',', $ids_to_turnon);
		$q = $steampressdb->query("update $steampressdb->links SET link_visible='Y' WHERE link_id IN ($all_linkson)");
	}
	...
	$all_links = join(',', $linkcheck);
	// should now have an array of links we can change
	$q = $steampressdb->query("update $steampressdb->links SET link_category='$category' WHERE link_id IN ($all_links)");
	...
	{
		$steampressdb->query("UPDATE $steampressdb->links set link_visible = 'N' WHERE link_category = $link_category");
	}
	$steampressdb->query("INSERT INTO $steampressdb->links (link_url, link_name, link_image, link_target, link_category, link_description, link_visible, link_owner, link_rating, link_rel, link_notes, link_rss) " .
		" VALUES('" . $link_url . "','"
			 . $link_name . "', '"
	...
	if (isset($submit))
	{
		...
		$steampressdb->query("UPDATE $steampressdb->links SET link_url='" . $link_url . "',
	...
	$steampressdb->query("DELETE FROM $steampressdb->links WHERE link_id = $link_id");
	...
	$link_id = (int) $_GET['link_id'];
	$row = $steampressdb->get_row("SELECT * 
...
<?php
	$sql = "SELECT link_url, link_name, link_image, link_description, link_visible,
			link_category AS cat_id, cat_name AS category, $steampressdb->users.user_login, link_id,
			link_rating, link_rel, $steampressdb->users.user_level
			FROM $steampressdb->links
			LEFT JOIN $steampressdb->linkcategories ON $steampressdb->links.link_category = $steampressdb->linkcategories.cat_id
			LEFT JOIN $steampressdb->users ON $steampressdb->users.ID = $steampressdb->links.link_owner ";
	...
	// echo "$sql";
	$links = $steampressdb->get_results($sql);
	if ($links)
	{
...
<?php
	$results = $steampressdb->get_results("SELECT ID, user_login FROM $steampressdb->users WHERE user_level > 0 ORDER BY ID");
