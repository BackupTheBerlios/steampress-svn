...
$steampressvarstoreset = array('action', 'safe_mode', 'withcomments', 'posts', 'poststart', 'postend', 'content', 'edited_post_title', 'comment_error', 'profile', 'trackback_url', 'excerpt', 'showcomments', 'commentstart', 'commentend', 'commentorder' );
...
case 'post':
	...
	$id_result = $steampressdb->get_row("SHOW TABLE STATUS LIKE '$steampressdb->posts'");
	...
	$postquery ="INSERT INTO $steampressdb->posts
	...
	$result = $steampressdb->query($postquery);
	...
	foreach ($post_categories as $post_category)
	{
		...
		$exists = $steampressdb->get_row("SELECT * FROM $steampressdb->post2cat WHERE post_id = $post_ID AND category_id = $post_category");
	...
	if ($post_status == 'static')
	{
		...
		add_post_meta($post_ID, '_steampress_page_template',  $_POST['page_template'], true);
	}
...
case 'edit':
	...
	$postdata = $steampressdb->get_row("SELECT * FROM $steampressdb->posts WHERE ID = '$post_ID'");
	...
	if ($post_status == 'static')
	{
		$page_template = get_post_meta($post_ID, '_steampress_page_template', true);
...
case 'editpost':
	...
	$old_categories = $steampressdb->get_col("SELECT category_id FROM $steampressdb->post2cat WHERE post_id = $post_ID");
	...
	foreach ($old_categories as $old_cat)
	{
		...
		{
			$steampressdb->query("DELETE FROM $steampressdb->post2cat WHERE category_id = $old_cat AND post_id = $post_ID LIMIT 1");
		}
	}
	...
	foreach ($post_categories as $new_cat)
	{
		...
		{
			$steampressdb->query("INSERT INTO $steampressdb->post2cat (post_id, category_id) VALUES ($post_ID, $new_cat)");
		}
	}
	...
	if ($post_status == 'static')
	{
		...
		if ( ! update_post_meta($post_ID, '_steampress_page_template',  $_POST['page_template']))
		{
			add_post_meta($post_ID, '_steampress_page_template',  $_POST['page_template'], true);
		}
	}
...
case 'delete':
	...
	if (! steampress_delete_post($post_id))
	{
	...
	{
		$sendback = get_settings('siteurl') .'/steampress-admin/post.php';
	}
...
case 'confirmdeletecomment':
	...
	echo "<form action='".get_settings('siteurl')."/steampress-admin/post.php' method='get'>\n";
	...
	echo "&nbsp;&nbsp;";
	echo "<input type=\"button\" value=\"" . __('No') . "\" onClick=\"self.location='". get_settings('siteurl') ."/steampress-admin/edit.php?p=$p&c=1#comments';\" />\n";
	echo "</form>\n";
	echo "</div>\n";
...
case 'deletecomment':
	...
	steampress_set_comment_status($comment, "delete");
	...
	{
		header('Location: '. get_settings('siteurl') .'/steampress-admin/edit.php?p='.$p.'&c=1#comments');
	}
	clear_smarty_cache();
...
case 'unapprovecomment':
	...
	steampress_set_comment_status($comment, "hold");
...
case 'mailapprovecomment':
	...
	{
		steampress_set_comment_status($comment, 'approve');
		if (true == get_option('comments_notify'))
		{
			steampress_notify_postauthor($comment);
		}
	}

	header('Location: ' . get_option('siteurl') . '/steampress-admin/moderation.php?approved=1');
...
case 'editedcomment':
	...
	$result = $steampressdb->query("
		UPDATE $steampressdb->comments SET
...
default:
	...
	if (user_can_create_draft($user_ID))
	{
		$action = 'post';
		get_currentuserinfo();
		$drafts = $steampressdb->get_results("SELECT ID, post_title FROM $steampressdb->posts WHERE post_status = 'draft' AND post_author = $user_ID");
		if ($drafts)
		{
			?>
			<div class="wrap">
