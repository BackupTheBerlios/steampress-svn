...
{
	if (!file_exists(ABSPATH . 'steampress-content/plugins/' . $check_plugin))
	{
			$current = get_settings('active_plugins');
...
<p><?php _e('Plugins are files you usually download separately from SteamPress that add functionality. To install a plugin you generally just need to put the plugin file into your <code>steampress-content/plugins</code> directory. Once a plugin is installed, you may activate it or deactivate it here. If something goes wrong with a plugin and you can&#8217;t use SteamPress, delete that plugin from the <code>steampress-content/plugins</code> directory and it will be automatically deactivated.'); ?></p>
<?php
