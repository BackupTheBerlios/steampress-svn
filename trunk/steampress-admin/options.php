...
case 'update':
	...
	$options = $steampressdb->get_results("SELECT $steampressdb->options.option_id, option_name, option_type, option_value, option_admin_level FROM $steampressdb->options WHERE option_name IN ($option_names)");
	...
	if ($options)
	{
		foreach ($options as $option)
		{
			...
			if ($user_level >= $option->option_admin_level)
			{
				...
				if ($new_val !== $old_val)
				{
					$result = $steampressdb->query("UPDATE $steampressdb->options SET option_value = '$new_val' WHERE option_name = '$option->option_name'");
				}
			}
		}
...
<div class="wrap">
  ...
  <table width="98%">
<?php
$options = $steampressdb->get_results("SELECT * FROM $steampressdb->options ORDER BY option_name");
