...
<p><a href="<?php echo get_settings('siteurl'); ?>/sp-commentsrss2.php?p=<?php echo $post->ID; ?>"><?php _e("<abbr title=\"Really Simple Syndication\">RSS</abbr> feed for comments on this post."); ?></a></p>
...
$comments = $spdb->get_results("SELECT * FROM $spdb->comments WHERE comment_post_ID = $id AND comment_approved = '1' ORDER BY comment_date");
$commentstatus = $spdb->get_row("SELECT comment_status, post_password FROM $spdb->posts WHERE ID = $id");
if (!empty($commentstatus->post_password) && $_COOKIE['sp-postpass_'. COOKIEHASH] != $commentstatus->post_password) {  // and it doesn't match the cookie
...
<form action="<?php echo get_settings('siteurl'); ?>/sp-comments-post.php" method="post" id="commentform">
	<p>
	  <input type="text" name="author" id="author" class="textarea" value="<?php echo $comment_author; ?>" size="28" tabindex="1" />
	   <label for="author"><?php _e("Name"); ?></label>
	<input type="hidden" name="comment_post_ID" value="<?php echo $id; ?>" />
	<input type="hidden" name="redirect_to" value="<?php echo sp_specialchars($_SERVER["REQUEST_URI"]); ?>" />
	</p>
...
<?php //} ?> 
<p class="credit"><?php timer_stop(1); ?> <?php echo sprintf(__("<cite>Powered by <a href=\"http://steampress.berlios.de\" title=\"%s\"><strong>Wordpress</strong></a></cite>"),__("Powered by SteamPress, state-of-the-art semantic personal publishing platform.")); ?></p>
