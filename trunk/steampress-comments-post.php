...
$post_status = $steampressdb->get_var("SELECT comment_status FROM $steampressdb->posts WHERE ID = '$comment_post_ID'");
...
$lasttime = $steampressdb->get_var("SELECT comment_date FROM $steampressdb->comments WHERE comment_author_IP = '$user_ip' ORDER BY comment_date DESC LIMIT 1");
if (!empty($lasttime)) {
...
$steampressdb->query("INSERT INTO $steampressdb->comments 
...
if (!$approved) {
	steampress_notify_moderator($comment_ID);
}

if ((get_settings('comments_notify')) && ($approved)) {
	steampress_notify_postauthor($comment_ID, 'comment');
}
