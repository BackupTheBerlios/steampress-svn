...
class sp_xmlrpc_server extends IXR_Server {

	function sp_xmlrpc_server() {
		$this->methods = array(
		  // Blogger API
	...
	function blogger_getPost($args) {
	  ...
	  $user_data = get_userdatabylogin($user_login);
	  $post_data = sp_get_single_post($post_ID, ARRAY_A);

	  $categories = implode(',', sp_get_post_cats(1, $post_ID));
	...
	function blogger_getRecentPosts($args) {
	  ...
	  $posts_list = sp_get_recent_posts($num_posts);
	  ...
	  foreach ($posts_list as $entry) {
	    ...
	    $categories = implode(',', sp_get_post_cats(1, $entry['ID']));
	...
	function blogger_newPost($args) {
	  ...
	  $post_ID = sp_insert_post($post_data);
	...
	function blogger_editPost($args) {
	  ...
	  $actual_post = sp_get_single_post($post_ID,ARRAY_A);
	  ...
	  $result = sp_update_post($postdata);
	...
	function mw_newPost($args) {
	  ...
	  $post_ID = sp_insert_post($postdata);
	...
	function mw_editPost($args) {
	  ...
	  $post_ID = sp_update_post($newpost);
	  if (!$post_ID) {
	...
	function mw_getPost($args) {
	  ...
	  if ($postdata['post_date'] != '') {
	    ...
	    $categories = array();
	    $catids = sp_get_post_cats('', $post_ID);
...
//	      'content' => $entry['post_content'],
	...
	function mw_getRecentPosts($args) {
	  ...
	  foreach ($posts_list as $entry) {
	    ...
	    $post_date = mysql2date('Ymd\TH:i:s', $entry['post_date']);
	    $categories = array();
	    $catids = sp_get_post_cats('', $entry['ID']);
	    foreach($catids as $catid) {
	      $categories[] = get_cat_name($catid);
	    }
...
//	      'content' => $entry['post_content'],
	...
	function mw_getCategories($args) {
	  ...
	  if ($cats = $spdb->get_results("SELECT cat_ID,cat_name FROM $spdb->categories", ARRAY_A)) {
	    foreach ($cats as $cat) {
	      $struct['categoryId'] = $cat['cat_ID'];
	      $struct['description'] = $cat['cat_name'];
	      $struct['categoryName'] = $cat['cat_name'];
	      $struct['htmlUrl'] = sp_specialchars(get_category_link(false, $cat['cat_ID'], $cat['cat_name']));
	      $struct['rssUrl'] = sp_specialchars(get_category_rss_link(false, $cat['cat_ID'], $cat['cat_name']));
	...
	function mw_newMediaObject($args) {
	  ...
	  // http://mycvs.org/archives/2004/06/30/file-upload-to-steampress-in-ecto/
	  ...
	  if(trim($file_realpath) == '' || trim($file_url) == '' ) {
	    // SteamPress is not correctly configured
	    logIO('O', '(MW) Bad configuration. Real/URL path not defined');
	    $this->error = new IXR_Error(500, 'Please configure SteamPress with valid paths for file upload.');
	    return $this->error;
	  }
	...
	function mt_getPostCategories($args) {
	  ...
	  $categories = array();
	  $catids = sp_get_post_cats('', intval($post_ID));
	...
	function mt_setPostCategories($args) {
	  ...
	  sp_set_post_cats('', $post_ID, $catids);
	...
	function mt_getTrackbackPings($args) {
	  ...
	  $comments = $spdb->get_results("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $spdb->comments WHERE comment_post_ID = $post_ID");
	...
	function pingback_ping($args) {
		...
		if ($post_ID = url_to_postid($pagelinkedto)) {
			...
			if (intval($urltest['fragment'])) {
				...
				$sql = "SELECT ID FROM $spdb->posts WHERE post_title RLIKE '$title'";
				if (! ($post_ID = $spdb->get_var($sql)) ) {
		...
		$sql = 'SELECT post_author FROM '.$spdb->posts.' WHERE ID = '.$post_ID;
		$result = $spdb->get_results($sql);
		...
		$result = $spdb->get_results("SELECT * FROM $spdb->comments WHERE comment_post_ID = '$post_ID' AND comment_author_url = '$pagelinkedfrom'");
		...
		$pingstatus = $spdb->get_var("SELECT ping_status FROM $spdb->posts WHERE ID = $post_ID");
		...
		$context = '[...] ';
		$context = sp_specialchars($original_context);
		$context .= ' [...]';
		$original_pagelinkedfrom = $pagelinkedfrom;
		$pagelinkedfrom = addslashes($pagelinkedfrom);
		$original_title = $title;

		$pingstatus = $spdb->get_var("SELECT ping_status FROM $spdb->posts WHERE ID = $tb_id");
		...
		do_action('pingback_post', $spdb->insert_id);
...
$sp_xmlrpc_server = new sp_xmlrpc_server();
