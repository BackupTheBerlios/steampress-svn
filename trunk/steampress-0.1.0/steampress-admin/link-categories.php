...
$steampressvarstoreset = array('action', 'cat', 'auto_toggle');
...
switch ($action)
{
	case 'addcat':
	{
		if ($user_level < 5)
		{
			...
			$steampressdb->query("INSERT INTO $steampressdb->linkcategories (cat_id, cat_name, auto_toggle, show_images, show_description, \n" .
	...
	case 'Delete':
	{
		...
		$steampressdb->query("DELETE FROM $steampressdb->linkcategories WHERE cat_id='$cat_id'");
		$steampressdb->query("UPDATE $steampressdb->links SET link_category=1 WHERE link_category='$cat_id'");
	...
	case 'Edit':
	{
		...
		$cat_id = $_GET['cat_id'];
		$row = $steampressdb->get_row("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
				 . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
				 . " text_after_all, list_limit FROM $steampressdb->linkcategories WHERE cat_id=$cat_id");
		if ($row)
		{
			if ($row->list_limit == -1)
			{
...
<?php
	...
	case "editedcat":
	{
		...
		$steampressdb->query("UPDATE $steampressdb->linkcategories set
						cat_name='$cat_name',
...
<?php
$results = $steampressdb->get_results("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
				 . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
				 . " text_after_all, list_limit FROM $steampressdb->linkcategories ORDER BY cat_id");
$i = 1;
