...
$steampressvarstoreset = array('action','item_ignored','item_deleted','item_approved');
		...
		if ($feelinglucky && 'later' == $value)
		{
			...
			switch($value)
			{
				case 'later':
					...
					// steampress_set_comment_status($key, "hold");
					++$item_ignored;
					break;
				
				case 'delete':
					steampress_set_comment_status($key, 'delete');
					++$item_deleted;
					break;
				
				case 'approve':
					steampress_set_comment_status($key, 'approve');
					if (get_settings('comments_notify') == true) {
						steampress_notify_postauthor($key);
					}
					++$item_approved;
					break;
			}
...
if ($user_level > 3)
{
	$comments = $steampressdb->get_results("SELECT * FROM $steampressdb->comments WHERE comment_approved = '0'");
}
else
{
	$comments = '';
