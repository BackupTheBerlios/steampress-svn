...
<?php
	$categories = $steampressdb->get_results("SELECT cat_id, cat_name, auto_toggle FROM $steampressdb->linkcategories ORDER BY cat_id");
...
<?php
				...
				if (isset($opml_url) && $opml_url != '')
				{
					...
					for ($i = 0; $i < $link_count; $i++)
					{
						...
						$query = "INSERT INTO $steampressdb->links (link_url, link_name, link_target, link_category, link_description, link_owner, link_rss)
								VALUES('{$urls[$i]}', '".addslashes($names[$i])."', '', $cat_id, '".addslashes($descriptions[$i])."', $user_ID, '{$feeds[$i]}')\n";
						$result = $steampressdb->query($query);
