...
if (! isset($steampress_did_header))
{
	...
	{
		die("There doesn't seem to be a <code>steampress-config.php</code> file. I need this before we can get started. Need more help? <a href='http://steampress.org/docs/faq/#steampress-config'>We got it</a>. You can <a href='steampress-admin/setup-config.php'>create a <code>steampress-config.php</code> file through a web interface</a>, but this doesn't work for all server setups. The safest way is to manually create the file.");
	}
	...
	elseif ( !isset($doing_rss) || !$doing_rss )
	{
	...
	if (1 == count($posts))
	{
		...
		if ( $s && empty($paged) && !strstr($_SERVER['PHP_SELF'], 'steampress-admin/'))
		{ // If they were doing a search and got one result
...
if ($pagenow == 'index.php')
{
	if (! isset($steampress_did_template_redirect))
	{
		...
		elseif (is_404() && file_exists("$steampress_template_dir/404.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/404.php");
			exit;
		}
		elseif (is_home() && file_exists("$steampress_template_dir/index.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/index.php");
			exit;
		}
		elseif (is_single() && file_exists("$steampress_template_dir/single.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/single.php");
			exit;
		}
		elseif (is_page() && file_exists(get_page_template()))
		{
			$steampress_did_template_redirect = true;
			include(get_page_template());
			exit;
		}
		elseif (is_category() &&  file_exists("$steampress_template_dir/category.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/category.php");
			exit;
		}
		elseif (is_author() && file_exists("$steampress_template_dir/author.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/author.php");
			exit;
		} elseif (is_date() && file_exists("$steampress_template_dir/date.php"))
		{
		...
		elseif (is_archive() && file_exists("$steampress_template_dir/archive.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/archive.php");
			exit;
		}
		elseif (is_search() && file_exists("$steampress_template_dir/search.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/search.php");
			exit;
		}
		elseif (is_paged() && file_exists("$steampress_template_dir/paged.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/paged.php");
			exit;
		}
		elseif (file_exists("$steampress_template_dir/index.php"))
		{
			$steampress_did_template_redirect = true;
			include("$steampress_template_dir/index.php");
			exit;
