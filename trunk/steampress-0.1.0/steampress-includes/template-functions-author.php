...
{
	global $steampressdb, $post, $querystring_start, $querystring_equal, $cache_userdata;
	$auth_ID = $author_id;
	...
	else
	{
		if ('' == $author_nicename)
		{
			$author_nicename = $cache_userdata[$author_id]->author_nicename;
		}
...
function get_author_rss_link($echo = false, $author_id, $author_nicename)
{
		...
		if ('' == $permalink_structure)
		{
			$file = get_settings('siteurl') . '/steampress-rss2.php';
...
function steampress_list_authors($args = '')
{
	parse_str($args, $r);
...
{
	global $steampressdb, $blogfilename;

	$query = "SELECT ID, user_nickname, user_firstname, user_lastname, user_nicename from $steampressdb->users " . ($exclude_admin ? "WHERE user_nickname <> 'admin' " : '') . "ORDER BY user_nickname";
	$authors = $steampressdb->get_results($query);
