...
function steampress_loginout() {
	global $user_level;
	get_currentuserinfo();

	if (0 == $user_level) :
		$link = '<a href="' . get_settings('siteurl') . '/steampress-login.php">' . __('Login') . '</a>';
	else :
		$link = '<a href="' . get_settings('siteurl') . '/steampress-login.php?action=logout">' . __('Logout') . '</a>';
	endif;

	echo apply_filters('loginout', $link);
}

function steampress_register() {
	global $user_level;

	get_currentuserinfo();

	if (0 == $user_level && get_settings('users_can_register') ) :
		$link = '<a href="' . get_settings('siteurl') . '/steampress-register.php">' . __('Register') . '</a>';
	else :
		$link = '<a href="' . get_settings('siteurl') . '/steampress-admin/">' . __('Site Admin') . '</a>';
	endif;

	echo apply_filters('register', $link);
}

function steampress_meta() {
	do_action('steampress_meta', 1);
}
...
function get_bloginfo($show='') {
	...
	case 'stylesheet_url':
		...
		if (empty($output) || $output == 'default') {
			$output = get_settings('siteurl') . "/steampress-layout.css";
		} else {
			$output = get_settings('siteurl') . "/steampress-content/themes/$output/style.css";
		}
		break;
	case 'stylesheet_directory':
		...
		if (empty($output) || $output == 'default') {
			...
			$output = get_settings('siteurl') . "/steampress-content/themes/$output";
		}
		break;
...
function steampress_title($sep = '&raquo;', $display = true) {
	global $steampressdb;
	...
	if (!empty($category_name)) {
		...
		$title = $steampressdb->get_var("SELECT cat_name FROM $steampressdb->categories WHERE category_nicename = '$category_name'");
	}
...
function single_post_title($prefix = '', $display = true) {
	...
	if (intval($p) || '' != $name) {
		if (!$p) {
			$p = $steampressdb->get_var("SELECT ID FROM $steampressdb->posts WHERE post_name = '$name'");
		}
...
function steampress_get_archives($args = '') {
	parse_str($args, $r);
...
function get_archives($type='', $limit='', $format='html', $before = '', $after = '', $show_post_count = false) {
	global $querystring_start, $querystring_equal, $querystring_separator, $month, $steampressdb;
	...
	if ('monthly' == $type) {
		$arcresults = $steampressdb->get_results("SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $steampressdb->posts WHERE post_date < '$now' AND post_status = 'publish' GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date DESC" . $limit);
		if ($arcresults) {
			$afterafter = $after;
			foreach ($arcresults as $arcresult) {
				$url  =  get_settings('siteurl').'/'.$arcresult->year.'/'. $arcresult->month;
				if ($show_post_count) {
		...
		$arcresults = $steampressdb->get_results("SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth` FROM $steampressdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
		if ($arcresults) {
			foreach ($arcresults as $arcresult) {
	...
	$start_of_week = get_settings('start_of_week');
		$arcresults = $steampressdb->get_results("SELECT DISTINCT WEEK(post_date, $start_of_week) AS `week`, YEAR(post_date) AS yr, DATE_FORMAT(post_date, '%Y-%m-%d') AS yyyymmdd FROM $steampressdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
		$arc_w_last = '';
		if ($arcresults) {
			foreach ($arcresults as $arcresult) {
		...
		$arcresults = $steampressdb->get_results("SELECT ID, post_date, post_title FROM $steampressdb->posts WHERE post_date < '$now' AND post_status = 'publish' ORDER BY post_date DESC" . $limit);
		if ($arcresults) {
			foreach ($arcresults as $arcresult) {
...
function get_calendar($daylength = 1) {
	global $steampressdb, $m, $monthnum, $year, $timedifference, $month, $month_abbrev, $weekday, $weekday_initial, $weekday_abbrev, $posts;

	// Quick check. If we have no posts at all, abort!
	if (!$posts) {
		$gotsome = $steampressdb->get_var("SELECT ID from $steampressdb->posts WHERE post_status = 'publish' ORDER BY post_date DESC LIMIT 1");
		if (!$gotsome)
			return;
	}
	...
	if (!empty($monthnum) && !empty($year)) {
		...
		$thismonth = $steampressdb->get_var("SELECT DATE_FORMAT((DATE_ADD('${thisyear}0101', INTERVAL $d DAY) ), '%m')");
	} elseif (!empty($m)) {
	...
	$previous = $steampressdb->get_row("SELECT DISTINCT MONTH(post_date) AS month, YEAR(post_date) AS year
			FROM $steampressdb->posts
			WHERE post_date < '$thisyear-$thismonth-01'
			AND post_status = 'publish'
							  ORDER BY post_date DESC
							  LIMIT 1");
	$next = $steampressdb->get_row("SELECT  DISTINCT MONTH(post_date) AS month, YEAR(post_date) AS year
			FROM $steampressdb->posts
			WHERE post_date >  '$thisyear-$thismonth-01'
	...
	echo '<table id="steampress-calendar">
	...
	// Get days with posts
	$dayswithposts = $steampressdb->get_results("SELECT DISTINCT DAYOFMONTH(post_date)
			FROM $steampressdb->posts WHERE MONTH(post_date) = $thismonth
	...
	$ak_post_titles = $steampressdb->get_results("SELECT post_title, DAYOFMONTH(post_date) as dom "
										 ."FROM $steampressdb->posts "
