...
Version: 2.6
Plugin URI: http://codex.steampress.berlios.de/Plugins/Staticize
...
define(CACHE_PATH, trailingslashit( ABSPATH . 'sp-content/staticize-cache' ) );
...
if ( !file_exists(CACHE_PATH) ) :
	if ( is_writable( dirname(CACHE_PATH) ) )
		...
		die("Your cache directory (<code>" . CACHE_PATH . "</code>) needs to be writable for this plugin to work. Double-check it. <a href='" . get_settings('siteurl') . "/sp-admin/plugins.php?action=deactivate&amp;plugin=staticize-reloaded.php'>Deactivate the Staticize Reloaded plugin</a>.");
endif;

// Array of files that have 'sp-' but should still be cached 
$acceptableFiles = array(
	'sp-atom.php', 'sp-comments-popup.php', 'sp-commentsrss2.php',
	'sp-links-opml.php', 'sp-locations.php', 'sp-rdf.php', 
	'sp-rss.php', 'sp-rss2.php'
);
...
function StaticizeInit() {
	...
	if( strstr($_SERVER['SCRIPT_NAME'], 'sp-') && !in_array($script, $acceptableFiles) ) 
		return;
...
function StaticizeCallback($buffer) {
	...
	if ( strstr($buffer, 'wpdberror') ) // If we detect a $spdb error
		return $buffer;
...
function staticize_header() {
	...
	$sp_last_modified = gmdate('D, d M Y H:i:s', $file_touched) . ' GMT';
