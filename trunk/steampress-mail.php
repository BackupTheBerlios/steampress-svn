...
for ($i=1; $i <= $count; $i++) :
		...
		if ($bodysignal) {
			...
			if (preg_match('/Subject: /i', $line)) {
				...
				if (!preg_match('#\=\?(.+)\?Q\?(.+)\?\=#i', $subject)) {
				  $subject = steampress_iso_descrambler($subject);
				}
			...
			if ( ereg("([a-zA-Z0-9\_\-\.]+@[\a-zA-z0-9\_\-\.]+)", $author , $regs) ) {
				...
				$result = $steampressdb->get_row("SELECT ID FROM $tableusers WHERE user_email='$regs[1]' ORDER BY ID DESC LIMIT 1");
				if (!$result)
					$post_author = 1;
				else
	...
	steampress_insert_post($post_data);
