...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
    global $spdb, $debug;
    $diffs = 0;
    $results = $spdb->get_results("DESC $table_name");
...
//check_column('sp_links', 'link_description', 'mediumtext'); 
//if (check_column($spdb->comments, 'comment_author', 'tinytext'))
//    echo "ok\n";
$error_count = 0;
$tablename = $spdb->links;
// check the column
if (!check_column($spdb->links, 'link_description', 'varchar(255)'))
{
    $ddl = "ALTER TABLE $spdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
    $q = $spdb->query($ddl);
}
if (check_column($spdb->links, 'link_description', 'varchar(255)')) {
    $res .= $tablename . ' - ok <br />';
