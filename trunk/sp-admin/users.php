...
case 'adduser':
	...
	$user_login     = sp_specialchars($_POST['user_login']);
	$pass1          = $_POST['pass1'];
	$pass2          = $_POST['pass2'];
	$user_email     = sp_specialchars($_POST['email']);
	$user_firstname = sp_specialchars($_POST['firstname']);
	$user_lastname  = sp_specialchars($_POST['lastname']);
	$user_uri       = sp_specialchars($_POST['uri']);
	...
	$user_ID = $spdb->get_var("SELECT ID FROM $spdb->users ORDER BY ID DESC LIMIT 1") + 1;
	...
	@sp_mail(get_settings('admin_email'), '[' . get_settings('blogname') . '] New User Registration', $message);
...
case 'promote':
	...
	if ('up' == $prom) {
		...
		$sql="UPDATE $spdb->users SET user_level=$new_level WHERE ID = $id AND $new_level < $user_level";
	} elseif ('down' == $prom) {
		$new_level = $usertopromote_level - 1;
		$sql="UPDATE $spdb->users SET user_level=$new_level WHERE ID = $id AND $new_level < $user_level";
	}
...
case 'delete':
	...
	$post_ids = $spdb->get_col("SELECT ID FROM $spdb->posts WHERE post_author = $id");
	if ($post_ids) {
		$post_ids = implode(',', $post_ids);
		
		// Delete comments, *backs
		$spdb->query("DELETE FROM $spdb->comments WHERE comment_post_ID IN ($post_ids)");
		// Clean cats
		$spdb->query("DELETE FROM $spdb->post2cat WHERE post_id IN ($post_ids)");
		// Clean post_meta
		$spdb->query("DELETE FROM $spdb->postmeta WHERE post_id IN ($post_ids)");
		// Clean links
		$spdb->query("DELETE FROM $spdb->links WHERE link_owner = $id");
		// Delete posts
		$spdb->query("DELETE FROM $spdb->posts WHERE post_author = $id");
	}

	// FINALLY, delete user
	$spdb->query("DELETE FROM $spdb->users WHERE ID = $id");
...
<div class="wrap">
  ...
  <table cellpadding="3" cellspacing="3" width="100%">
	...
	<?php
	$users = $spdb->get_results("SELECT ID FROM $spdb->users WHERE user_level > 0 ORDER BY ID");
	$style = '';
	foreach ($users as $user) {
		...
		$numposts = $spdb->get_var("SELECT COUNT(*) FROM $spdb->posts WHERE post_author = $user->ID and post_status = 'publish'");
...
<?php
$users = $spdb->get_results("SELECT * FROM $spdb->users WHERE user_level = 0 ORDER BY ID");
if ($users) {
?>
<div class="wrap">
...
<?php printf(__('<p>Users can <a href="%s/sp-register.php">register themselves</a> or you can manually create users here.</p>'), get_settings('siteurl')); ?>
