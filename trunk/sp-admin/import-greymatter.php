...
$spvarstoreset = array('action', 'gmpath', 'archivespath', 'lastentry');
...
<link rel="stylesheet" href="sp-admin/b2.css" type="text/css">
<style type="text/css">
<!--
<?php
...
<p><font face="times new roman" style="font-size: 39px;">gm 2 <img src="../sp-images/wpminilogo.png" width="50" height="50" border="0" alt="SP" align="top" /></font></p>
...
<li>importing users... <ul><?php
	...
	foreach($userbase as $user) {
		...
		$loginthere = $spdb->get_var("SELECT user_login FROM $spdb->users WHERE user_login = '$user_login'");
		if ($loginthere) {
			echo "<li>user <i>$user_login</i>... <b>Already exists</b></li>";
			continue;
		}

		$query = "INSERT INTO $spdb->users (user_login,user_pass,user_nickname,user_email,user_url,user_ip,user_domain,user_browser,dateYMDhour,user_level,user_idmode) VALUES ('$user_login','$pass1','$user_nickname','$user_email','$user_url','$user_ip','$user_domain','$user_browser','$user_joindate','1','nickname')";
...
<li>importing posts, comments, and karma...<br /><ul><?php
	...
	for($i = 0; $i <= $lastentry; $i = $i + 1) {
		...
		if (is_file($entryfile.".cgi")) {
			...
			$sql = "SELECT * FROM $spdb->users WHERE user_login = '$post_author'";
			...
			if (!mysql_num_rows($result)) { // if deleted from GM, we register the author as a level 0 user in b2
				...
				$user_url=addslashes("");
				$user_joindate=addslashes($user_joindate);
				$query = "INSERT INTO $spdb->users (user_login,user_pass,user_nickname,user_email,user_url,user_ip,user_domain,user_browser,dateYMDhour,user_level,user_idmode) VALUES ('$user_login','$pass1','$user_nickname','$user_email','$user_url','$user_ip','$user_domain','$user_browser','$user_joindate','0','nickname')";
				$result = mysql_query($query);
				if ($result==false) {
					die ("<strong>ERROR</strong>: couldn't register an user!");
				}
			...
			$query = "INSERT INTO $spdb->posts (post_author,post_date,post_content,post_title) VALUES ('$post_author_ID','$post_date','$post_content','$post_title')";
			$result = mysql_query($query) or die(mysql_error());

			if (!$result)
				die ("Error in posting...");
			
			$sql2 = "SELECT * FROM $spdb->posts WHERE 1=1 ORDER BY ID DESC LIMIT 1";
			$result2 = mysql_query($sql2);
			$myrow2 = mysql_fetch_array($result2);
			$post_ID=$myrow2[0];

			// Grab a default category.
			$post_category = $spdb->get_var("SELECT cat_ID FROM $spdb->categories LIMIT 1");

			// Update the post2cat table.
			$exists = $spdb->get_row("SELECT * FROM $spdb->post2cat WHERE post_id = $post_ID AND category_id = $post_category");
			...
			if ($c>4) {
				for ($j=4;$j<$c;$j++) {
					...
					$sql3 = "INSERT INTO $spdb->comments (comment_post_ID,comment_author,comment_author_email,comment_author_url,comment_author_IP,comment_date,comment_content) VALUES ('$comment_post_ID','$comment_author','$comment_author_email','$comment_author_url','$comment_author_IP','$comment_date','$comment_content')";
...
<p>Now you can go and <a href="sp-login.php">log in</a>, have fun !</p>
	<?php
	break;
...
<body style="margin: 20px;">
<p><font face="times new roman" style="font-size: 39px;">gm 2 <img src="../sp-images/wpminilogo.png" width="50" height="50" border="0" alt="SP" align="top" /></font></p>
<p>This is a basic GreyMatter to SteamPress import script.</p>
<p>What it does:</p>
<ul>
<li>parses gm-authors.cgi to import authors: everyone is imported at level 1</li>
<li>parses the entries cgi files to import posts, comments, and karma on posts (although karma is not used on SteamPress yet)<br />if authors are found not to be in gm-authors.cgi, imports them at level 0</li>
</ul>
...
<h3>First step: install SteamPress</h3>
<p>Install the SteamPress blog as explained in the <a href="../readme.html" target="_blank">ReadMe</a>, then immediately come back here.</p>
