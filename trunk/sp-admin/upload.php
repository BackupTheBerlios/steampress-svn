...
case 'not-writable':
?>
<p><?php printf(__("It doesn't look like you can use the file upload feature at this time because the directory you have specified (<code>%s</code>) doesn't appear to be writable by SteamPress. Check the permissions on the directory and for typos."), get_settings('fileupload_realpath')) ?></p>
...
die();
    ...
    if($_POST['thumbsize'] != 'none' ) {
        ...
        $result = sp_create_thumbnail($pathtofile, $max_side, NULL);
        if($result != 1) {
            print $result;
        }
    }
