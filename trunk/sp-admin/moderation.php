...
$spvarstoreset = array('action', 'item_ignored', 'item_deleted', 'item_approved', 'feelinglucky');
	...
	if ($feelinglucky && 'later' == $value)
	    ...
	    switch($value) {
			case 'later':
				...
				// sp_set_comment_status($key, "hold");
				++$item_ignored;
				break;
			
			case 'delete':
				sp_set_comment_status($key, 'delete');
				++$item_deleted;
				break;
			
			case 'approve':
				sp_set_comment_status($key, 'approve');
				if (get_settings('comments_notify') == true) {
					sp_notify_postauthor($key);
				}
				++$item_approved;
				break;
	    }
...
<?php
if ($user_level > 3)
	$comments = $spdb->get_results("SELECT * FROM $spdb->comments WHERE comment_approved = '0'");
else
	$comments = '';
