...
if (1 < $user_level) {
	$editable = $spdb->get_col("SELECT ID FROM $spdb->users WHERE user_level <= '$user_level' AND ID != $user_ID");
        if( is_array( $editable ) == false )
            $editable = array( "1" );
	$editable = join(',', $editable);
	$other_drafts = $spdb->get_results("SELECT ID, post_title FROM $spdb->posts WHERE post_status = 'draft' AND post_author IN ($editable) ");
} else {
	$other_drafts = false;
}
...
if ( $_GET['m'] ) {
	...
	printf(__('Search for &#8220;%s&#8221;'), sp_specialchars($_GET['s']) );
} else {
	_e('Last 15 Posts');
}
?>
...
<form name="viewarc" action="" method="get">
    ...
    <select name='m'>
	<?php
		$arc_result=$spdb->get_results("SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $spdb->posts ORDER BY post_date DESC");
...
include(ABSPATH.'sp-blog-header.php');
...
if ( 1 == count($posts) ) {
	...
	$comments = $spdb->get_results("SELECT * FROM $spdb->comments WHERE comment_post_ID = $id ORDER BY comment_date");
	if ($comments) {
	?> 
  ...
  <?php 
			if (($user_level > $authordata->user_level) or ($user_login == $authordata->user_login)) {
				...
				if ( ('none' != $comment_status) && ($user_level >= 3) ) {
					if ('approved' == sp_get_comment_status($comment->comment_ID)) {
