...
$spvarstoreset = array('action', 'safe_mode', 'withcomments', 'posts', 'poststart', 'postend', 'content', 'edited_post_title', 'comment_error', 'profile', 'trackback_url', 'excerpt', 'showcomments', 'commentstart', 'commentend', 'commentorder' );
...
case 'post':
	...
	$id_result = $spdb->get_row("SHOW TABLE STATUS LIKE '$spdb->posts'");
	...
	$postquery ="INSERT INTO $spdb->posts
	...
	if ($post_status == 'static') {
		...
		add_post_meta($post_ID, '_sp_page_template',  $_POST['page_template'], true);
	}
...
case 'edit':
	...
	$postdata = $spdb->get_row("SELECT * FROM $spdb->posts WHERE ID = '$post_ID'");
	...
	if ($post_status == 'static') {
		$page_template = get_post_meta($post_ID, '_sp_page_template', true);
...
case 'editpost':
	...
	$old_categories = $spdb->get_col("SELECT category_id FROM $spdb->post2cat WHERE post_id = $post_ID");
	
	// Delete any?
	foreach ($old_categories as $old_cat) {
		if (!in_array($old_cat, $post_categories)) // If a category was there before but isn't now
			$spdb->query("DELETE FROM $spdb->post2cat WHERE category_id = $old_cat AND post_id = $post_ID LIMIT 1");
	}
	
	// Add any?
	foreach ($post_categories as $new_cat) {
		if (!in_array($new_cat, $old_categories))
			$spdb->query("INSERT INTO $spdb->post2cat (post_id, category_id) VALUES ($post_ID, $new_cat)");
	}
	...
	if ($post_status == 'static') {
		...
		if ( ! update_post_meta($post_ID, '_sp_page_template',  $_POST['page_template'])) {
			add_post_meta($post_ID, '_sp_page_template',  $_POST['page_template'], true);
		}
	}
...
case 'delete':
	...
	if (! sp_delete_post($post_id))
		die(__('Error in deleting...'));

	$sendback = $_SERVER['HTTP_REFERER'];
	if (strstr($sendback, 'post.php')) $sendback = get_settings('siteurl') .'/sp-admin/post.php';
...
case 'confirmdeletecomment':
	...
	echo "<form action='".get_settings('siteurl')."/sp-admin/post.php' method='get'>\n";
	...
	echo "&nbsp;&nbsp;";
	echo "<input type=\"button\" value=\"" . __('No') . "\" onclick=\"self.location='". get_settings('siteurl') ."/sp-admin/edit.php?p=$p&amp;c=1#comments';\" />\n";
	echo "</form>\n";
	echo "</div>\n";
...
case 'deletecomment':
	...
	sp_set_comment_status($comment, "delete");
	do_action('delete_comment', $comment);

	if (($_SERVER['HTTP_REFERER'] != "") && (false == $noredir)) {
		header('Location: ' . $_SERVER['HTTP_REFERER']);
	} else {
		header('Location: '. get_settings('siteurl') .'/sp-admin/edit.php?p='.$p.'&c=1#comments');
	}
...
case 'unapprovecomment':
	...
	sp_set_comment_status($comment, "hold");
...
case 'mailapprovecomment':
	...
	if ('1' != $commentdata['comment_approved']) {
		sp_set_comment_status($comment, 'approve');
		if (true == get_option('comments_notify'))
			sp_notify_postauthor($comment);
	}

	header('Location: ' . get_option('siteurl') . '/sp-admin/moderation.php?approved=1');
...
default:
	...
	if (user_can_create_draft($user_ID)) {
		$action = 'post';
		get_currentuserinfo();
		$drafts = $spdb->get_results("SELECT ID, post_title FROM $spdb->posts WHERE post_status = 'draft' AND post_author = $user_ID");
		if ($drafts) {
			?>
			<div class="wrap">
		...
		$content = sp_specialchars($content);
