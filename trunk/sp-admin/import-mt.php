<?php
define('MTEXPORT', '');// enter the relative path of the import.txt file containing the mt entries. If the file is called import.txt and it is /sp-admin, then this line
//should be define('MTEXPORT', 'import.txt');

if (!file_exists('../sp-config.php')) die("There doesn't seem to be a sp-config.php file. You must install SteamPress before you import any entries.");
...
<title>SteamPress &rsaquo; Import from Movable Type</title>
...
<style media="screen" type="text/css">
	...
	#logo {
		margin: 0;
		padding: 0;
		background-image: url(http://steampress.berlios.de/images/logo.png);
...
<h1 id="logo"><a href="http://steampress.berlios.de">SteamPress</a></h1> 
<?php
switch($step) {

	case 0:
?> 
<p>Howdy! We&#8217;re about to begin the process to import all of your Movable Type entries into SteamPress. Before we get started, you need to edit this file (<code>import-mt.php</code>) and change one line so we know where to find your MT export file. To make this easy put the import file into the <code>sp-admin</code> directory. Look for the line that says:</p>
...
function users_form($n) {
	global $spdb, $testing;
	$users = $spdb->get_results("SELECT * FROM $spdb->users ORDER BY ID");
...
<p><?php _e('Below, you can see the names of the authors of the MovableType posts in <i>italics</i>. For each of these names, you can either pick an author in your SteamPress installation from the menu, or enter a name for the author in the textbox.'); ?></p>
<p><?php _e('If a new user is created by SteamPress, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)'); ?></p>
	<?php
	echo '<ol id="authors">';
	...
	global $spdb, $mtnames, $newauthornames, $j;//mtnames is an array with the names in the mt import file
	...
	if (!(in_array($author, $mtnames))) { //a new mt author name is found
		...
		$user_id = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_login = '$newauthornames[$j]'"); //check if the new author name defined by the user is a pre-existing wp user
		if (!$user_id) { //banging my head against the desk now. 
			if ($newauthornames[$j] == 'left_blank') { //check if the user does not want to change the authorname
				$spdb->query("INSERT INTO $spdb->users (user_level, user_login, user_pass, user_nickname) VALUES ('1', '$author', '$md5pass', '$author')"); // if user does not want to change, insert the authorname $author
				$user_id = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_login = '$author'");
				$newauthornames[$j] = $author; //now we have a name, in the place of left_blank.
			} else {
			$spdb->query("INSERT INTO $spdb->users (user_level, user_login, user_pass, user_nickname) VALUES ('1', '$newauthornames[$j]', '$md5pass', '$newauthornames[$j]')"); //if not left_blank, insert the user specified name
			$user_id = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_login = '$newauthornames[$j]'");
			}
    ...
    $user_id = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_login = '$newauthornames[$key]'");//use that key to get the value of the author's name from $newauthornames
	}
	return $user_id;
...
//				echo "\n$key: $value";
	...
	if ($spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_title = '$post_title' AND post_date = '$post_date'")) {
	    ...
	    $spdb->query("INSERT INTO $spdb->posts (
		...
		$post_id = $spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_title = '$post_title' AND post_date = '$post_date'");
		if (0 != count($post_categories)) {
			foreach ($post_categories as $post_category) {
			// See if the category exists yet
			$cat_id = $spdb->get_var("SELECT cat_ID from $spdb->categories WHERE cat_name = '$post_category'");
			if (!$cat_id && '' != trim($post_category)) {
				$cat_nicename = sanitize_title($post_category);
				$spdb->query("INSERT INTO $spdb->categories (cat_name, category_nicename) VALUES ('$post_category', '$cat_nicename')");
				$cat_id = $spdb->get_var("SELECT cat_ID from $spdb->categories WHERE cat_name = '$post_category'");
			}
			if ('' == trim($post_category)) $cat_id = 1;
			// Double check it's not there already
			$exists = $spdb->get_row("SELECT * FROM $spdb->post2cat WHERE post_id = $post_id AND category_id = $cat_id");
			...
			$exists = $spdb->get_row("SELECT * FROM $spdb->post2cat WHERE post_id = $post_id AND category_id = 1");
			if (!$exists) $spdb->query("INSERT INTO $spdb->post2cat (post_id, category_id) VALUES ($post_id, 1) ");
		}
		...
		if ('' != trim($comment)) {
			...
			if (!$spdb->get_row("SELECT * FROM $spdb->comments WHERE comment_date = '$comment_date' AND comment_content = '$comment_content'")) {
				$spdb->query("INSERT INTO $spdb->comments (comment_post_ID, comment_author, comment_author_email, comment_author_url, comment_author_IP, comment_date, comment_content, comment_approved)
				VALUES
