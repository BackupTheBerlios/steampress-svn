...
<title>SteamPress &rsaquo; Textpattern Import</title>
...
switch($step) {
	...
	case 0:
?> 
<p>This script imports your entries from Textpattern into SteamPress. It should be relatively painless, and we hope you're happy with the result.</p>
...
$query = "ALTER TABLE `$spdb->posts` ADD `post_name` VARCHAR(200) NOT NULL";
maybe_add_column($spdb->posts, 'post_name', $query);
...
$author = $spdb->get_var("SELECT ID FROM $spdb->users WHERE user_level = 10 LIMIT 1");
$category = $spdb->get_var("SELECT cat_ID FROM $spdb->categories LIMIT 1");
...
while ($post = mysql_fetch_array($posts)) {
	...
	// Get steampress post id
	$sp_post_ID = $spdb->get_var("SELECT ID FROM $spdb->posts ORDER BY ID DESC LIMIT 1");
	...
	if ($comments) {
		while($comment = mysql_fetch_object($comments)) {
			...
			$spdb->query("INSERT INTO $spdb->comments
				...
				VALUES
				($sp_post_ID, '$comment->name', '$comment->email', '$comment->web', '$comment->ip', '$comment->posted', '$comment->message')");
		}
	}
}
