...
<?php
	$categories = $spdb->get_results("SELECT cat_id, cat_name, auto_toggle FROM $spdb->linkcategories ORDER BY cat_id");
                ...
                if (isset($opml_url) && $opml_url != '') {
                    ...
                    for ($i = 0; $i < $link_count; $i++) {
                        ...
                        if ('http' == substr($titles[$i], 0, 4))
                            $titles[$i] = '';
                        $query = "INSERT INTO $spdb->links (link_url, link_name, link_target, link_category, link_description, link_owner, link_rss)
                                VALUES('{$urls[$i]}', '".addslashes($names[$i])."', '', $cat_id, '".addslashes($descriptions[$i])."', $user_ID, '{$feeds[$i]}')\n";
                        $result = $spdb->query($query);
