...
$spvarstoreset = array('action', 'redirect', 'profile', 'user_id');
...
$new_user_login  = sp_specialchars($_POST['new_user_login']);
$new_firstname   = sp_specialchars($_POST['new_firstname']);
$new_lastname    = sp_specialchars($_POST['new_lastname']);
$new_nickname    = $_POST['new_nickname'];
$new_nicename    = sanitize_title($new_nickname, $user_id);
$new_icq         = sp_specialchars($_POST['new_icq']);
$new_aim         = sp_specialchars($_POST['new_aim']);
$new_msn         = sp_specialchars($_POST['new_msn']);
$new_yim         = sp_specialchars($_POST['new_yim']);
$new_email       = sp_specialchars($_POST['new_email']);
$new_url         = sp_specialchars($_POST['new_url']);
$new_url         = preg_match('/^(https?|ftps?|mailto|news|gopher):/is', $new_url) ? $new_url : 'http://' . $new_url; 
$new_idmode      = sp_specialchars($_POST['new_idmode']);
$new_description = $_POST['new_description'];

$result = $spdb->query("UPDATE $spdb->users SET user_login = '$new_user_login', user_firstname = '$new_firstname', $updatepassword user_lastname='$new_lastname', user_nickname='$new_nickname', user_icq='$new_icq', user_email='$new_email', user_url='$new_url', user_aim='$new_aim', user_msn='$new_msn', user_yim='$new_yim', user_idmode='$new_idmode', user_description = '$new_description', user_nicename = '$new_nicename' WHERE ID = $user_id");
