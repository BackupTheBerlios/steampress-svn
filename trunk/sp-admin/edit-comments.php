...
else $mode = sp_specialchars($_GET['mode'], 1);
?>
...
<form name="searchform" action="" method="get"> 
  ...
  <input type="text" name="s" value="<?php if (isset($_GET['s'])) echo sp_specialchars($_GET['s'], 1); ?>" size="17" /> 
...
if ( !empty( $_POST['delete_comments'] ) ) :
	...
	foreach ($_POST['delete_comments'] as $comment) : // Check the permissions on each
		$comment = (int) $comment;
		$post_id = $spdb->get_var("SELECT comment_post_ID FROM $spdb->comments WHERE comment_ID = $comment");
		$authordata = get_userdata( $spdb->get_var("SELECT post_author FROM $spdb->posts WHERE ID = $post_id") );
		if ( user_can_delete_post_comments($user_ID, $post_id) ) :
			$spdb->query("DELETE FROM $spdb->comments WHERE comment_ID = $comment");
			++$i;
		endif;
	endforeach;
...
if (isset($_GET['s'])) {
	$s = $spdb->escape($_GET['s']);
	$comments = $spdb->get_results("SELECT * FROM $spdb->comments  WHERE
	...
	$comments = $spdb->get_results("SELECT * FROM $spdb->comments ORDER BY comment_date DESC LIMIT $offset,20");
}
if ('view' == $mode) {
	if ($comments) {
		if ($offset)
		...
		++$i; $class = '';
		$authordata = get_userdata($spdb->get_var("SELECT post_author FROM $spdb->posts WHERE ID = $comment->comment_post_ID"));
			$comment_status = sp_get_comment_status($comment->comment_ID);
...
?>		
        ...
        <p><?php _e('Posted'); echo ' '; comment_date('M j, g:i A');  
			...
			if ( user_can_edit_post($user_ID, $comment->comment_post_ID) ) {
				$post_title = $spdb->get_var("SELECT post_title FROM $spdb->posts WHERE ID = $comment->comment_post_ID");
