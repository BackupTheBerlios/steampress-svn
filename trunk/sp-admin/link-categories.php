...
switch ($action) {
  ...
  case 'Edit':
  {
    ...
    $row = $spdb->get_row("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
         . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
         . " text_after_all, list_limit FROM $spdb->linkcategories WHERE cat_id=$cat_id");
    if ($row) {
        if ($row->list_limit == -1) {
            $row->list_limit = '';
        }
...
<div class="wrap">
  <h2>Edit &#8220;<?php echo sp_specialchars($row->cat_name)?>&#8221; Category </h2>
...
<tr>
	...
	<td width="67%"><input name="cat_name" type="text" value="<?php echo sp_specialchars($row->cat_name)?>" size="30" /></td>
</tr>
<tr>
...
<tr>
	...
	<td width="67%"><input type="text" name="text_before_link" size="45" value="<?php echo sp_specialchars($row->text_before_link)?>" /></td>
</tr>
<tr>
<th scope="row"><?php _e('Between Link and Description:') ?></th>
<td><input type="text" name="text_after_link" size="45" value="<?php echo sp_specialchars($row->text_after_link)?>" /></td>
</tr>
<tr>
<th scope="row"><?php _e('After Link:') ?></th>
<td><input type="text" name="text_after_all" size="45" value="<?php echo sp_specialchars($row->text_after_all)?>"/></td>
</tr>
</table>
</fieldset>
...
<div class="wrap">
            ...
            <table cellpadding="5" cellspacing="0" border="0"  class="largedata">
              <tr>
...
$results = $spdb->get_results("SELECT cat_id, cat_name, auto_toggle, show_images, show_description, "
         . " show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, "
         . " text_after_all, list_limit FROM $spdb->linkcategories ORDER BY cat_id");
$i = 1;
...
?>
              <tr valign="middle" align="center" <?php echo $style ?> style="border-bottom: 1px dotted #9C9A9C;">
                <td><?php echo sp_specialchars($row->cat_name)?></td>
