...
<title>SteamPress &rsaquo; Import from RSS</title>
...
if (!$content) : // This is for feeds that put content in description
	...
	$content = $spdb->escape( unhtmlentities( trim($content[1]) ) );
endif;
...
if ('' == $title) : 
	$dupe = $spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_content = '$content' AND post_date = '$post_date'");
else :
	$dupe = $spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_title = '$title' AND post_date = '$post_date'");
endif;
...
if ($dupe) :
	...
	$post_id = $spdb->get_var("SELECT ID FROM $spdb->posts WHERE post_title = '$title' AND post_date = '$post_date'");
