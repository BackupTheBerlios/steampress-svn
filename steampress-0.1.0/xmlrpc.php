...
class steampress_xmlrpc_server extends IXR_Server
{

	function steampress_xmlrpc_server()
	{
		$this->methods = array(
			// Blogger API
	...
	function blogger_getPost($args)
	{
		...
		$user_data = get_userdatabylogin($user_login);
		$post_data = steampress_get_single_post($post_ID, ARRAY_A);

		$categories = implode(',', steampress_get_post_cats(1, $post_ID));
	...
	function blogger_getRecentPosts($args)
	{
		...
		$posts_list = steampress_get_recent_posts($num_posts);
		...
		foreach ($posts_list as $entry)
		{
			...
			$categories = implode(',', steampress_get_post_cats(1, $entry['ID']));
	...
	function blogger_newPost($args)
	{
		...
		$post_ID = steampress_insert_post($post_data);
	...
	function blogger_editPost($args)
	{
		...
		$actual_post = steampress_get_single_post($post_ID,ARRAY_A);
		...
		$result = steampress_update_post($postdata);
	...
	function blogger_deletePost($args)
	{
		...
		$result = steampress_delete_post($post_ID);
	...
	function mw_newPost($args)
	{
		...
		$post_ID = steampress_insert_post($postdata);
	...
	function mw_editPost($args)
	{
		...
		$post_ID = steampress_update_post($newpost);
		if (!$post_ID)
		{
	...
	function mw_getPost($args)
	{
		...
		if ($postdata['post_date'] != '')
		{
			...
			$categories = array();
			$catids = steampress_get_post_cats('', $post_ID);
...
//				'content' => $entry['post_content'],
	...
	function mw_getRecentPosts($args)
	{
		...
		foreach ($posts_list as $entry)
		{
			...
			$post_date = mysql2date('Ymd\TH:i:s', $entry['post_date']);
			$categories = array();
			$catids = steampress_get_post_cats('', $entry['ID']);
			foreach($catids as $catid)
			{
				$categories[] = get_cat_name($catid);
			}
...
//				'content' => $entry['post_content'],
	...
	function mw_getCategories($args)
	{
		...
		if ($cats = $steampressdb->get_results("SELECT cat_ID,cat_name FROM $steampressdb->categories", ARRAY_A))
		{
			foreach ($cats as $cat)
			{
	...
	function mt_getPostCategories($args) {
		...
		$categories = array();
		$catids = steampress_get_post_cats('', intval($post_ID));
	...
	function mt_setPostCategories($args)
	{
		...
		steampress_set_post_cats('', $post_ID, $catids);
	...
	function mt_getTrackbackPings($args)
	{
		...
		$comments = $steampressdb->get_results("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $steampressdb->comments WHERE comment_post_ID = $post_ID");
	...
	function mt_publishPost($args)
	{
		...
		// retain old cats
		$cats = steampress_get_post_cats('',$post_ID);
	...
	function pingback_ping($args)
	{
		...
		global $steampressdb, $steampress_version; 
		...
		elseif (isset($urltest['fragment']))
		{
			...
			elseif (is_string($urltest['fragment']))
			{
				...
				$sql = "SELECT ID FROM $steampressdb->posts WHERE post_title RLIKE '$title'";
				if (! ($post_ID = $steampressdb->get_var($sql)) )
				{
		...
		$sql = 'SELECT post_author FROM '.$steampressdb->posts.' WHERE ID = '.$post_ID;
		$result = $steampressdb->get_results($sql);

		if (!$steampressdb->num_rows)
		{
			// Post_ID not found
				return new IXR_Error(33, 'The specified target URI cannot be used as a target. It either doesn\'t exist, or it is not a pingback-enabled resource.');
		}
		...
		$sql = 'SELECT * FROM '.$steampressdb->comments.' 
...
//return($sql);
		...
		$pingstatus = $steampressdb->get_var("SELECT ping_status FROM $steampressdb->posts WHERE ID = $post_ID");
		...
		steampress_new_comment($commentdata);

		$comment_ID = $steampressdb->insert_id;
...
$steampress_xmlrpc_server = new steampress_xmlrpc_server();
