...
{
	$output = '<form action="' . get_settings('siteurl') . '/steampress-pass.php" method="post">
...
function get_the_content($more_link_text = '(more...)', $stripteaser = 0, $more_file = '')
{
	...
	if (!empty($post->post_password))
	{ // if there's a password
		if (stripslashes($_COOKIE['steampress-postpass_'.COOKIEHASH]) != $post->post_password)
...
function get_the_excerpt($fakeit = true)
{
	...
	if (!empty($post->post_password))
	{ // if there's a password
		if ($_COOKIE['steampress-postpass_'.COOKIEHASH] != $post->post_password)
		{  // and it doesn't match the cookie
...
function steampress_link_pages($args = '')
{
	parse_str($args, $r);
	if (!isset($r['before']))
	{
...
function steampress_list_pages($args = '')
{
	global $steampressdb;
	...
	$pages = $steampressdb->get_results("SELECT ID, post_title FROM $steampressdb->posts WHERE post_status = 'static' ORDER BY post_" . $r['sort_column'] . " " . $r['sort_order'] = 'asc');
