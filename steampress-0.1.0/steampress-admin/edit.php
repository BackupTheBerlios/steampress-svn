...
if (1 < $user_level)
{
	$editable = $steampressdb->get_col("SELECT ID FROM $steampressdb->users WHERE user_level <= '$user_level' AND ID != $user_ID");
		if( is_array( $editable ) == false )
		{
			$editable = array( "1" );
		}
	$editable = join(',', $editable);
	$other_drafts = $steampressdb->get_results("SELECT ID, post_title FROM $steampressdb->posts WHERE post_status = 'draft' AND post_author IN ($editable) ");
}
else
{
	$other_drafts = false;
...
<form name="viewarc" action="" method="get" style="float: left; width: 20em; margin-bottom: 1em;">
	...
	<select name='m'>
	<?php
		$arc_result=$steampressdb->get_results("SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $steampressdb->posts ORDER BY post_date DESC");
...
include(ABSPATH.'steampress-blog-header.php');
...
if ( 1 == count($posts) )
{
	$comments = $steampressdb->get_results("SELECT * FROM $steampressdb->comments WHERE comment_post_ID = $id ORDER BY comment_date");
	if ($comments)
	{
	?> 
...
{
$comment_status = steampress_get_comment_status($comment->comment_ID);
?> 
...
<li <?php if ("unapproved" == $comment_status) { echo "class='unapproved'"; } ?> >
	...
	<?php 
			if (($user_level > $authordata->user_level) or ($user_login == $authordata->user_login))
			{
				...
				{
					if ('approved' == steampress_get_comment_status($comment->comment_ID))
					{
