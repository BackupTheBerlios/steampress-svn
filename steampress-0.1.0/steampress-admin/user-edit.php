...
$steampressvarstoreset = array('action', 'redirect', 'profile', 'user_id');
...
if ($edituser->user_level >= $user_level)
{
...
$result = $steampressdb->query("UPDATE $steampressdb->users SET user_login = '$new_user_login', user_firstname = '$new_firstname', $updatepassword user_lastname='$new_lastname', user_nickname='$new_nickname', user_icq='$new_icq', user_email='$new_email', user_url='$new_url', user_aim='$new_aim', user_msn='$new_msn', user_yim='$new_yim', user_idmode='$new_idmode', user_description = '$new_description', user_nicename = '$new_nicename' WHERE ID = $user_id");
...
<table width="99%"  border="0" cellspacing="2" cellpadding="3">
	...
	<tr>
		...
		<td><select name="new_idmode">
				<option value="nickname"<?php if ($edituser->user_idmode == 'nickname')
				{
					echo ' selected="selected"';
				}
				?>><?php echo $edituser->user_nickname ?></option>
				<option value="login"<?php if ($edituser->user_idmode=="login")
				{
					echo ' selected="selected"';
				}
				?>><?php echo $edituser->user_login ?></option>
				<option value="firstname"<?php if ($edituser->user_idmode=="firstname")
				{
					echo ' selected="selected"';
				}
				?>><?php echo $edituser->user_firstname ?></option>
				<option value="lastname"<?php if ($edituser->user_idmode=="lastname")
				{
					echo ' selected="selected"';
				}
				?>><?php echo $edituser->user_lastname ?></option>
				<option value="namefl"<?php if ($edituser->user_idmode=="namefl")
				{
					echo ' selected="selected"';
				?>><?php echo $edituser->user_firstname." ".$edituser->user_lastname ?></option>
				}
				<option value="namelf"<?php if ($edituser->user_idmode=="namelf")
				{
					echo ' selected="selected"';
				}
				?>><?php echo $edituser->user_lastname." ".$edituser->user_firstname ?></option>
			</select>
		</td>
	</tr>
	<tr>
