...
if ( !empty( $_POST['delete_comments'] ) )
{
	...
	{
		$comment = (int) $comment;
		$post_id = $steampressdb->get_var("SELECT comment_post_ID FROM $steampressdb->comments WHERE comment_ID = $comment");
		$authordata = get_userdata( $steampressdb->get_var("SELECT post_author FROM $steampressdb->posts WHERE ID = $post_id") );
		if ( user_can_delete_post_comments($user_ID, $post_id) )
		{
			$steampressdb->query("DELETE FROM $steampressdb->comments WHERE comment_ID = $comment");
			++$i;
		}
	}
...
if (isset($_GET['s']))
{
	$s = $steampressdb->escape($_GET['s']);
	$comments = $steampressdb->get_results("SELECT * FROM $steampressdb->comments  WHERE
...
else
{
	...
	$comments = $steampressdb->get_results("SELECT * FROM $steampressdb->comments ORDER BY comment_date DESC LIMIT $offset,20");
}
if ('view' == $mode)
{
	if ($comments)
	{
		...
		{
		++$i; $class = '';
		$authordata = get_userdata($steampressdb->get_var("SELECT post_author FROM $steampressdb->posts WHERE ID = $comment->comment_post_ID"));
...
?>		
        ...
        <p><?php _e('Posted'); echo ' '; comment_date('M j, g:i A');  
			...
			if ( user_can_edit_post($user_ID, $comment->comment_post_ID) ) {
				$post_title = $steampressdb->get_var("SELECT post_title FROM $steampressdb->posts WHERE ID = $comment->comment_post_ID");
